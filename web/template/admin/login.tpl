...
<input type="text" name="username" id="username"  onblur="changeusername(this.value,'u');" {{if $nametype eq 'realname'}}disabled='disabled'{{/if}}  style="width: {{if !$logintype.ldapauth and !$logintype.radiusauth and !$logintype.adauth or !$loginauthtype or !$loginauthtype}}155{{else}}100{{/if}}px;{{if !$dpwdtime}}height:24px;{{/if}}{{if $nametype eq 'realname'}}display:none{{/if}}">
<input type="text" name="username" id="realname"  onblur="changeusername(this.value,'r');" {{if $nametype eq 'username' or !$nametype}}disabled='disabled'{{/if}}  style="width: {{if !$logintype.ldapauth and !$logintype.radiusauth and !$logintype.adauth or !$loginauthtype or !$loginauthtype}}155{{else}}100{{/if}}px;{{if !$dpwdtime}}height:24px;{{/if}}{{if $nametype eq 'username'}}display:none{{/if}}">

{{else}}
<select name='username' id="username"  style="width: {{if !$logintype.ldapauth and !$logintype.radiusauth and !$logintype.adauth or !$loginauthtype or !$loginauthtype}}155{{else}}200{{/if}}px;{{if !$dpwdtime}}height:24px;{{/if}}">
...
<select name='username' id="realname" disabled style="width: {{if !$logintype.ldapauth and !$logintype.radiusauth and !$logintype.adauth or !$loginauthtype or !$loginauthtype}}155{{else}}200{{/if}}px;{{if !$dpwdtime}}height:24px;{{/if}}display:none">
...
{{else}}
<select name='authtype' id="authtype" style="width:95px;{{if !$dpwdtime}}height:24px;{{/if}}">
...
{{/if}}
                ...
                <TR>
                  ...
                  <TD><INPUT name="password" id="textfield2" autocomplete="off" style="width: {{if !$logintype.ldapauth and !$logintype.radiusauth and !$logintype.adauth or !$loginauthtype or !$loginauthtype}}155{{else}}200{{/if}}px;{{if !$dpwdtime}}height:24px;{{/if}}" type="password"></TD>
                </TR>
				{{if $dpwdtime}}
                <TR>
                  <TD height="30" align="right" bgcolor="#edf6f6">动态密码：</TD>
                  <TD><INPUT name="dpassword" autocomplete="off" id="textfield2" style="width: {{if !$logintype.ldapauth and !$logintype.radiusauth and !$logintype.adauth or !$loginauthtype or !$loginauthtype}}155{{else}}200{{/if}}px; color: rgb(153, 153, 153);" onFocus="if(value==defaultValue){value='';this.style.color='#000'}" onBlur="if(!value){value=defaultValue;this.style.color='#999'}" type="text" value="令牌用户输入"></TD>
                </TR>
                <TR>
      ...
      <tr>
        <td height="35" align="center"><input name="button" type="submit" class="{{if 0&&$dpwdtime}}dl_btn1{{else}}btn1{{/if}}" id="button" value="登 录"></td>
      </tr>
    </table></TD>
  </TR>
  </FORM></TABLE>
