...
// pid : parentID.
dTree.prototype.getChildren = function(pid, gname, ouname, cname, dn){
	...
	$.get(url, {"1":Math.round(new Date().getTime()/1000)}, function (data, textStatus){
		...
		for(;i<aNodes['cns'].length;i++){
			...
			tree.add(pid+'_'+i, pid, '<input type="checkbox" id="u_'+aNodes['pid']+'_'+i+'" onclick="checkgroup(this.checked,\''+aNodes['pid']+'_'+i+'\')" name="cns[]" value="'+name+'" >'+name, "javascript:"+tree.obj+".getChildren(\'"+aNodes['pid']+'_'+i+"\','','',\'"+name+"\',\'"+aNodes['dn']+"\')", name,'',tree.icon.folder);
		}
		for(;i<aNodes['cns'].length+aNodes['ous'].length;i++){
			var name=aNodes['ous'][i-aNodes['cns'].length];
			tree.add(pid+'_'+i, pid, '<input type="checkbox" id="u_'+aNodes['pid']+'_'+i+'" onclick="checkgroup(this.checked,\''+aNodes['pid']+'_'+i+'\')" name="ous[]" value="'+name+'" >'+name, "javascript:"+tree.obj+".getChildren(\'"+aNodes['pid']+'_'+i+"\','',\'"+name+"\','',\'"+aNodes['dn']+"\')", name,'',tree.icon.folder);
		...
		for(;i<aNodes['cns'].length+aNodes['ous'].length+aNodes['groups'].length;i++){
			var name=aNodes['groups'][i-aNodes['ous'].length-aNodes['cns'].length];
			tree.add(pid+'_'+i, pid, '<input type="checkbox" id="u_'+aNodes['pid']+'_'+i+'" onclick="checkgroup(this.checked,\''+aNodes['pid']+'_'+i+'\')" name="groups[]" onclick="ddev.o('+pid+''+i+');" value="'+name+'" >'+name, "javascript:"+tree.obj+".getChildren(\'"+aNodes['pid']+'_'+i+"\','','',\'"+name+"\',\'"+aNodes['dn']+"\')", name,'',tree.icon.folder);
		...
		for(;i<aNodes['cns'].length+aNodes['ous'].length+aNodes['groups'].length+aNodes['users'].length;i++){
			...
			tree.add(pid+''+i, pid, '<input type="checkbox" id="u_'+aNodes['pid']+'_'+i+'" name="username[]" value="'+name+'" >'+(aNodes['users'][i-aNodes['ous'].length-aNodes['groups'].length-aNodes['cns'].length]['exists']==1 ? '<font style="color:red">' : '')+name+(aNodes['users'][i-aNodes['ous'].length-aNodes['groups'].length-aNodes['cns'].length]['exists']==1 ? '</font>' : ''), "#", name,'',tree.icon.node);
