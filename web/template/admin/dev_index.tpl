...
{{else}}
	...
	{{/if}}
{{/if}}
</ul>{{if $from eq 'dir'}}<span class="back_img"><A href="admin.php?controller=admin_pro&action=dev_group&back=1"><IMG src="{{$template_root}}/images/back1.png" 
	...
	<td class="" colspan = "7"><table width="100%" border="0" cellspacing="0" cellpadding="0"  class="main_content">
                ...
                <TBODY>
				 <TR>
					...
					&nbsp;&nbsp;<input  type="submit" value=" 搜索 " onclick="return searchit();" class="bnnew2">&nbsp;&nbsp;显示空目录<input type="checkbox" name="showemptydir" onclick="window.parent.menu.window.showNodeCount0(this.checked)"  />&nbsp;&nbsp;&nbsp;&nbsp;目录截取<input type="checkbox" name="showemptydir" onclick="window.parent.menu.window.showLongTitle(this.checked)" checked />
			...
			<tr  {{if $alldev[t].ct > 0 or ($alldev[t].asset_warrantdate ne '0000-00-00 00:00:00' && $alldev[t].warrantdays<0) }}bgcolor="red" {{elseif  ($alldev[t].asset_warrantdate ne '0000-00-00 00:00:00' && $alldev[t].warrantdays<30)}}bgcolor="yellow"{{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}   onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if $alldev[t].ct > 0 or ($alldev[t].asset_warrantdate ne '0000-00-00 00:00:00' && $alldev[t].warrantdays<0) }}red{{elseif  ($alldev[t].asset_warrantdate ne '0000-00-00 00:00:00' && $alldev[t].warrantdays<30)}}yellow{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">
				...
				<td>
					...
					| <img src='{{$template_root}}/images/left_dot1.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_config&action=apppub_list&device_ip={{$alldev[t].device_ip}}&serverid={{$alldev[t].id}}&gid={{$gid}}'>应用({{if $alldev[t].appct}}{{$alldev[t].appct}}{{else}}0{{/if}})</a>
					
					{{*| <img src='{{$template_root}}/images/left_dot1.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'><a href='admin.php?controller=admin_pro&action=accountlinux_index&ip={{$alldev[t].device_ip}}&serverid={{$alldev[t].id}}&gid={{$gid}}&accountlinux=1'>账号信息({{if $alldev[t].appuserct}}{{$alldev[t].appuserct}}{{else}}0{{/if}})</a>*}}
			...
			{{/section}}
			  ...
			  <input name="select_all" id="select_all" type="checkbox" onClick="javascript:for(var i=0;i<this.form.elements.length;i++){var e=document.member_list.elements[i];if(e.name=='chk_member[]')e.checked=document.member_list.select_all.checked;}" value="checkbox">&nbsp;&nbsp; {{if $smarty.session.ADMIN_LEVEL eq 1 or $smarty.session.ADMIN_LEVEL eq 3 or $smarty.session.ADMIN_LEVEL eq 21  or $smarty.session.ADMIN_LEVEL eq 4}}
			   <input type="button"  value="添加" onClick="location.href='admin.php?controller=admin_pro&action=dev_edit&gid={{$gid}}'" class="an_02">
			   &nbsp;&nbsp;<input type="button"  value="批量添加" onClick="location.href='admin.php?controller=admin_pro&action=devbatchadd&gid={{$gid}}'" class="an_02">
			   &nbsp;&nbsp;<input type="submit"  value="批量删除" onClick="return my_confirm('删除所选');" class="an_02">
			   &nbsp;&nbsp;<input type="submit"  value="批量修改" onClick="document.member_list.action='admin.php?controller=admin_pro&action=batchserverpriorityedit&gid={{$gid}}';" class="an_02">
			   &nbsp;&nbsp;<input type="button"  value="导入" onClick="location.href='admin.php?controller=admin_pro&action=devimport&gid={{$gid}}'" class="an_02">		
				...
				<input type="button"  value="添加已有资源" onClick="location.href='admin.php?controller=admin_pro&action=groupadddev&gid={{$gid}}&from={{$from}}'" class="an_06">
				{{/if}}
				{{/if}}
				{{/if}}
