...
<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_workflow&action=workflow_approve">流程审批</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li> 
</ul>
</div></td></tr>
	...
	<form name="ip_list" action="admin.php?controller=admin_workflow&action=workflow_delete" method="post">
                <TBODY>
                  <TR>
                    ...
                    <th class="list_bg"  width="25%"><a href="admin.php?controller=admin_workflow&action=workflow_approve&orderby1=desc&orderby2={{$orderby2}}" >描述</a></TD>
			...
			<tr  {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}} onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">
				...
				<img src="{{$template_root}}/images/list_ico1.gif" width="16" height="16" align="absmiddle">{{if $s[t].approved eq 1}}已同意 {{if $s[t].apply_status_reject eq 1}}<a href="#" onclick="loadurl('admin.php?controller=admin_workflow&action=approvedesc&wid={{$s[t]['sid']}}&status=2');">驳回批准</a>{{/if}}{{elseif $s[t].approved eq 2}}驳回{{elseif $s[t].apply_status_priority}}<a href="#" onclick="loadurl('admin.php?controller=admin_workflow&action=approvedesc&wid={{$s[t]['sid']}}&status=1');">批准</a>  <a href="#" onclick="loadurl('admin.php?controller=admin_workflow&action=approvedesc&wid={{$s[t]['sid']}}&status=2');">驳回</a>{{/if}}
				</td> 
			</tr>
			{{/section}}
	          <tr>
	           <td  colspan="3" align="left">&nbsp;&nbsp;&nbsp;&nbsp; 
				  {{if $smarty.session.ADMIN_LEVEL == 0}}
					<input type="button" name="submit" onclick="location='admin.php?controller=admin_workflow&action=workflow_edit'" value=" 新建 " class="an_02" />
				  {{/if}}
		   		</td>
