...
<ul>
  ...
  <tr>
	...
	<TD colspan="1" align="left">
		...
		{{include file="select_sgroup.tpl" }}    运维用户<input type="text" class="wbk" name="webuser" value="{{$webuser}}">
...
<td></td><td></td></tr>
	  ...
	  <tr><td align="right" style="border-bottom:0px;border-top:0px;border-left:0px;border-right:0px;">全选<input type="checkbox" value=2 onclick="checkAll(this.checked);"></td></tr>
		...
		<td width="180"><input type="checkbox" name='member[]' value='{{$allmem[g].uid}}'  {{$allmem[g].check}}>{{*<a onclick="window.open ('admin.php?controller=admin_config&action=apppubedit_seluser&memberid={{$allmem[g].uid}}&apppubid={{$p.id}}&sessionluser={{$sessionluser}}', 'newwindow', 'height=330, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=no, resizable=yes,location=no, status=no');return false;" href="#" target="_blank" >*}}{{$allmem[g].username}}({{if $allmem[g].realname}}{{$allmem[g].realname}}{{else}}未设置{{/if}}){{*</a>*}}</td>{{if ($smarty.section.g.index +1) % 5 == 0}}</tr><tr>{{/if}}
		{{/if}}
		{{/section}}
		</tr></table>
	  </td>
