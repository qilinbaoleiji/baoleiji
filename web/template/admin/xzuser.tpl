...
<META name="author" content="nuttycoder"> <LINK href="{{$template_root}}/all_purpose_style.css" 
rel="stylesheet" type="text/css"> 
	<link rel="StyleSheet" href="{{$template_root}}/cssjs/dtree.css" type="text/css" />
...
function selectgroup(groupid){
	...
	for(var i=0; i<window.opener.AllMember.length; i++){
		if(window.opener.AllMember[i]['groupid']==groupid){
			useroptions.options[useroptions.options.length] = new Option(window.opener.AllMember[i]['username']+'('+window.opener.AllMember[i]['realname']+')', 'user_'+window.opener.AllMember[i]['uid']);
			if(window.opener.AllMember[i]['check']=='checked'){
...
function filter(){
	...
	useroptions.options.length=0;
	for(var i=0; i<window.opener.AllMember.length; i++){
		if(/*window.opener.AllMember[i]['groupid']==selectedgroup &&*/ window.opener.AllMember[i]['username']==username3){
			useroptions.options[useroptions.options.length] = new Option(window.opener.AllMember[i]['username']+'('+window.opener.AllMember[i]['realname']+')', 'user_'+window.opener.AllMember[i]['uid']);
			if(window.opener.AllMember[i]['check']=='checked'){
				useroptions.options[useroptions.options.length-1].style.color="red";
			}
...
function createdtree(filter){
	...
	if(window.opener.servergroup!=null)
	for(var i=0; i<window.opener.servergroup.length; i++){
		if(filter.length==0 || (filter.length>0&&window.opener.servergroup[i].name==filter))
		d.add(window.opener.servergroup[i].id,window.opener.servergroup[i].ldapid,window.opener.servergroup[i].name,'javascript:selectgroup('+window.opener.servergroup[i].id+');');
	}
              ...
              <tr>
                <td width="75%" valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0" class=BBtable>
                  ...
                  <tr>
                    <td>
                   <div  class="dtreecob"> 
                    <div class="dtree" id="dtree" style="overflow:auto; width:100%;height:260px;">
		<script type="text/javascript">
