...
</ul><span class="back_img"><A href="admin.php?controller=admin_pro&action=resource_group&back=1"><IMG src="{{$template_root}}/images/back1.png" 
...
<input type="hidden" name="oldgname" value="{{$ginfo.groupname}}">
	  <tr>
  <td align="right" colspan="1">系统用户组:<input type=text size="50" name="gname" id="gname" value="{{$ginfo.groupname}}"> </td><td align="left" colspan="2">描述:<input type=text size="50" name="desc" id="desc" value="{{$ginfo.desc}}"></td>
  <tr>
	  <tr>
	  <td width="45%" align=right>
		...
		<div class="select_move_2">				
				...
				&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{include file="select_sgroup.tpl" }}<br /><br /><br />
		...
		<option value="groupid_{{$res2[r2].groupid}}_{{$res2[r2].username}}" title="{{$res2[r2].groupname}}_{{$res2[r2].groupid}}_{{$res2[r2].username}}" selected>设备组_{{$res2[r2].groupname}}_{{$res2[r2].groupid}}_{{$res2[r2].username}}</option>
		{{/section}}
...
function reload(p1,p2,check){
	...
	function selectgroup(groupid,groupname){
		...
		for(var i=0; i<inputs.length; i++){
			if(inputs[i].name=='agroupid1_group[]'&&inputs[i].checked){
...
//document.getElementById('agroupid1_group_{{$res2[i].groupid}}').checked = true;
selectedgroup[{{$smarty.section.i.index}}] = {{$res2[i].groupid}};
{{/section}}
function checkS(){
	var inputs = document.getElementsByTagName("input");
	for(var i=0; i<inputs.length; i++){
		if(inputs[i].name=='agroupid1_group[]'){
			inputs[i].checked=false;
		}
	}
	for(i=0; i<selectedgroup.length; i++){
		document.getElementById('agroupid1_group_'+selectedgroup[i]).checked = true;
	}
}
checkS();
</script>
