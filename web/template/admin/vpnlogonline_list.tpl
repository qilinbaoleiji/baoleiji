...
<ul>
	...
	<li class="me_a"><img src="{{$template_root}}/images/an1.jpg" align="absmiddle"/><a href="admin.php?controller=admin_vpnlog&action=online">在线用户</a><img src="{{$template_root}}/images/an3.jpg" align="absmiddle"/></li>
</ul>
</div></td></tr>
  <tr>
         <td >
	...
	<td class="">
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
					...
					<tr>
						...
						<th class="list_bg"  bgcolor="d9ecfa" width="10%"><a href="admin.php?controller=admin_vpnlog&action=online&orderby1=ip&orderby2={{$orderby2}}" >目标ip</a></th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%"><a href="admin.php?controller=admin_vpnlog&action=online&orderby1=in_time&orderby2={{$orderby2}}" >开始时间</a></th>
						<th class="list_bg"  bgcolor="d9ecfa" width="10%"><a href="admin.php?controller=admin_vpnlog&action=online&orderby1=out_time&orderby2={{$orderby2}}" >结束时间</a></th>
					...
					<tr {{if $allsession[t].dangerous > 1}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td><a href="admin.php?controller=admin_vpnlog&action=online&user={{$allsession[t].user}}">{{$allsession[t].user}}</a></td>
						<td><a href="admin.php?controller=admin_vpnlog&action=online&fromip={{$allsession[t].src_ip}}">{{$allsession[t].src_ip}}</a></td>
						<td><a href="admin.php?controller=admin_vpnlog&action=online&toip={{$allsession[t].ip}}">{{$allsession[t].ip}}</a></td>
						<td>{{$allsession[t].in_time}}</ td>
						<td>{{$allsession[t].out_time}}</td>
						<td><img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_vpnlog&action=delon&id={{$allsession[t].id}}">删除</a></td>
					</tr>
