...
servergroup[i++]={id:{{$allsgroup[a].id}},name:'{{$allsgroup[a].groupname}}',ldapid:{{$allsgroup[a].ldapid}},level:{{$allsgroup[a].level}},attribute:{{$allsgroup[a].attribute}}};
{{/section}}
var i=0;
  ...
  <tr>
	<td class="">
		...
		<form name="f1" method=post action="admin.php?controller=admin_reports&action=cronreport_save&id={{$defaultp.id}}">
         ...
         <tr bgcolor="">
		<TD width="10%" align=right>报表模板 </TD><td colspan="2"><input type="radio" name='template' value="html" {{if $defaultp.template eq 'html' or $defaultp.template eq ''}}checked{{/if}}>HTML&nbsp;&nbsp;&nbsp;<input type="radio" name='template' value="excel" {{if $defaultp.template eq 'excel'}}checked{{/if}}>EXCEL&nbsp;&nbsp;&nbsp;<input type="radio" name='template' value="pdf" {{if $defaultp.template eq 'pdf'}}checked{{/if}}>PDF</td>
		</tr>
{{assign var="trnumber" value=0}}
		<tr bgcolor="f7f7f7">
		<TD width="10%" align=right rowspan="4">日发送的报表 </TD>
