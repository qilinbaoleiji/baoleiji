...
function search(){
	...
	if(form.hostname==undefined){
		...
		form.action += "&appname="+encodeURIComponent(form.appname.value);
	}else{
		form.action += "&sip="+form.sip.value;
...
应用名称：<input type="text" class="wbk" id="appname" name="appname" value="{{$appname}}" style="width:80px;">&nbsp;IP：<input type="text" class="wbk" id="sip" name="sip" value="{{$sip}}" style="width:80px;" >
{{else}}
...
{{/section}}
</select>&nbsp;<span id="slogin_template" style="display:none">登录方式：<select  class="wbk"  id="app_act" style="width:80px;">
...
{{/if}}&nbsp;<input type="submit" height="35" align="middle" value=" 确定 " border="0" class="bnnew2"/>{{if $smarty.session.ADMIN_LEVEL eq 0}}&nbsp;磁盘映射:<input type="checkbox" name="rdpdiskauth_" id="rdpdiskauth_" {{if !$member.rdpdiskauth_up}}disabled{{/if}} {{if $rdpdiskauth_up}}checked{{/if}} value="1"  />&nbsp;剪切版:<input type="checkbox" name="rdpclipauth_" id="rdpclipauth_" {{if !$member.rdpclipauth_up }}disabled{{/if}}  onclick="checkieNT52(this)" {{if $member.rdpclipauth_up }}checked{{/if}} value="1"  />&nbsp;本地:<input type="checkbox" name="consoleauth_" id="consoleauth_" {{if !$member.rdplocal}}disabled{{else}}{{if $member.rdplocalcheck }}checked{{/if}}{{/if}}  />&nbsp;<select  class="wbk"  name='fenbianlv' id='fenbianlv' > 
					...
					</select>&nbsp;域名连接:<input type="checkbox" name="bydomain" id="bydomain" {{if $bydomain}}checked{{/if}} value="1"  />{{/if}}
</form>
	  </td>
  </tr>
  <tr>
...
<form name='usrdev' >
		...
		{{if  $logintype ne 'apppub' and $logintype ne '_apppub'}}		
			...
			<tr bgcolor='{{if !$alldev[t].enable }}#cccccc{{elseif $alldev[t].passwordtry eq 1 or $alldev[t].passwordtry eq 2}}red{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}' onmouseover="changeStyle(this,'o');" onmouseout="changeStyle(this,'{{if !$alldev[t].enable }}#cccccc{{elseif $alldev[t].passwordtry eq 1 or $alldev[t].passwordtry eq 2}}red{{elseif $smarty.section.t.index % 2 == 0}}f7f7f7{{/if}}');">{{if $smarty.session.ADMIN_LEVEL eq 1}}
				...
				<td align="center"><a href='#' onclick='loadurl("admin.php?controller=admin_pro&action=showdesc&id={{$alldev[t].id}}");return false;' target="hide"><img src='{{$template_root}}/images/1-1.gif' width=16 height='16' hspace='5' border='0' align='absmiddle'>{{$alldev[t].desc|truncate_cn:5:"...":""}}</a></td>
				{{if $type ne 'fort'}}
				...
				<td class="td_line" width="30%">
					{{if $admin_level == 0}}					
					{{if  $alldev[t].login_method eq 'RDP' or $alldev[t].login_method eq 'X11'}}					{{if $alldev[t].enable and $alldev[t].twopriority}}
					<a id="a{{$alldev[t].id}}1" onclick="rdpgo({{$alldev[t].id}},0,true);return false;" href='admin.php?controller=admin_pro&rdptype=activex&action=dev_login&id={{$alldev[t].id}}' ><img src="{{$template_root}}/images/mstsc_ico.gif" border=0 title="MSTSC:{{$alldev[t].connections}}"></a>{{if  $windows_version ne '5.2'}}&nbsp;&nbsp;&nbsp;<a id="a{{$alldev[t].id}}2" onclick="rdpgo2({{$alldev[t].id}},0,true)" href='#' target="_blank"><img src="{{$template_root}}/images/ie_ico.png" title="ACTIVEX" border=0></a>{{/if}}					{{else}}
					<img src="{{$template_root}}/images/mstsc_ico.gif" border=0 title="MSTSC:{{$alldev[t].connections}}">&nbsp;&nbsp;&nbsp;<img src="{{$template_root}}/images/ie_ico.png" title="ACTIVEX" border=0>					{{/if}}
					...
					<a id="p{{$alldev[t].id}}" href="admin.php?controller=admin_pro&action=dev_login&id={{$alldev[t].id}}&logintool=putty&type={{$type}}" onclick="return goto3(this.id)" target="hide"><img src="{{$template_root}}/images/putty_ico.gif" title="PUTTY" border=0></a>&nbsp;&nbsp;&nbsp;
					 ...
					 <a id="s{{$alldev[t].id}}" href="admin.php?controller=admin_pro&action=dev_login&id={{$alldev[t].id}}&logintool=securecrt&type={{$type}}" onclick="return goto3(this.id)"  target="hide" ><img src="{{$template_root}}/images/scrt_ico.gif" title="SECURECRT" border=0></a></a>
					 {{else}}
					...
					{{if $alldev[t].enable and $alldev[t].twopriority}}
					<a id="a{{$alldev[t].id}}1" onclick="rdpgo({{$alldev[t].id}},0,false);return false;" href='admin.php?controller=admin_pro&rdptype=activex&action=dev_login&id={{$alldev[t].id}}'><font style="font-size:12px;" {{if $alldev[t].puttyhong eq 1}}color="red"{{/if}} ><img src="{{$template_root}}/images/mstsc_ico.gif" title="MSTSC" border=0></font></a>&nbsp;&nbsp;&nbsp;{{if  $windows_version ne '5.2'}}<a id="a{{$alldev[t].id}}2" onclick="rdpgo2({{$alldev[t].id}},0,false)" href='#' target="_blank"><img src="{{$template_root}}/images/ie_ico.png" title="ACTIVEX" border=0></a>
{{/if}}					{{else}}
...
{{/if}}
	           <td colspan="10" align="right">
		   			{{if $smarty.session.ADMIN_LEVEL eq 10}}<input type="button"  value="密码打印" onClick="alert('未发现可驱动的密码打印机');" class="an_06">{{/if}}&nbsp;&nbsp;&nbsp;{{if $logintype ne 'apppub'}}共{{$total}}个记录{{/if}}  {{$page_list}}  页次：{{$curr_page}}/{{$total_page}}页  {{if $logintype ne 'apppub'}}{{$items_per_page}}个记录/页{{/if}}  转到第<input name="pagenum" type="text" class="wbk" size="2" onKeyPress="if(event.keyCode==13) {window.location='{{$curr_url}}&page='+this.value;return false;}">页
		   </td>
		</tr>
		</TBODY>
              </TABLE>
