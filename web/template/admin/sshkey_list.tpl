...
.ul li{ width:80px; float:left;}
  ...
  <tr>
	...
	<tr bgcolor="f7f7f7">
		...
		{{include file="select_sgroup.tpl" }}              &nbsp;&nbsp;&nbsp;IP：<input type="text" name="ip" id="ip" >&nbsp;&nbsp;&nbsp;主账号：<input type="text" name="username" id="username" >&nbsp;&nbsp;&nbsp;<input type="button" onclick="searchip();" value="确定" /></div>
	  </td>
	  </tr><tr>
...
function searchip(){
	...
	{{if $_config.LDAP}}
	ldapid1=document.getElementById("ldapid1").options[document.getElementById("ldapid1").options.selectedIndex].value;
	ldapid2=document.getElementById("ldapid2").options[document.getElementById("ldapid2").options.selectedIndex].value;
	ldapid3=document.getElementById("ldapid3").options[document.getElementById("ldapid3").options.selectedIndex].value;
	ldapid4=document.getElementById("ldapid4").options[document.getElementById("ldapid4").options.selectedIndex].value;
	ldapid5=document.getElementById("ldapid5").options[document.getElementById("ldapid5").options.selectedIndex].value;
	{{/if}}
	groupid=document.getElementById("groupid").options[document.getElementById("groupid").options.selectedIndex].value;
	if(changed){
		if(confirm('确定要放弃更改?')){
			window.location='admin.php?controller=admin_pro&action=sshkey_list&ldapid1='+ldapid1+'&ldapid2='+ldapid2+'&ldapid3='+ldapid3+'&ldapid4='+ldapid4+'&ldapid5='+ldapid5+'&groupid='+groupid+'&gname={{$ginfo.groupname}}&id={{$ginfo.id}}'+'&ip='+ip+'&username='+username;
		}
	}else{
		window.location='admin.php?controller=admin_pro&action=sshkey_list&ldapid1='+ldapid1+'&ldapid2='+ldapid2+'&ldapid3='+ldapid3+'&ldapid4='+ldapid4+'&ldapid5='+ldapid5+'&groupid='+groupid+'&gname={{$ginfo.groupname}}&id={{$ginfo.id}}'+'&ip='+ip+'&username='+username;
	}
