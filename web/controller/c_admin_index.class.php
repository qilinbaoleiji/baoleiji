...
class c_admin_index extends c_base {
	...
	function index() {		
		...
		if($user['webportal']){
			$this->assign("sessionlifetime", $user['webportallogin']-mktime());
		}else{
			$this->assign("sessionlifetime", $pwdconfig['logintimeout']*60+$_SESSION['startonlinetime']-mktime());
		}
	...
	function menu() {
		...
		if($_CONFIG['priority_cache'] || $user['searchcache']){
			$appservers = $this->appmember_set->base_select("SELECT d.device_ip appserverip,a.name hostname,s.groupid FROM ".$this->devices_cache_set->get_table_name()." d  LEFT JOIN ".$this->server_set->get_table_name()." s ON d.device_ip=s.device_ip left join apppserver a ON s.device_ip=a.appserverip  WHERE memberid=".$_SESSION['ADMIN_UID']." AND d.login_method=26 and a.id is not null GROUP BY d.device_ip", 'hostname', 'asc');		
		...
		if($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101){
			$where .= " AND (id IN(".$_SESSION['ADMIN_MSERVERGROUP'].") or id IN(".implode(',', $_SESSION['ADMIN_MSERVERGROUP_IDS']).") or id IN(".implode(',', $_SESSION['ADMIN_MSERVERGROUP_PARENT_IDS']).") ) ";
		}
		...
		if($_SESSION['ADMIN_LEVEL']==0){
			...
			if($_SESSION['ADMIN_LEVEL']==1){
				$sql = "SELECT sg.*,IFNULL(t.count,0) count,IFNULL(tt.count,0) mcount FROM ".$this->sgroup_set->get_table_name()." sg LEFT JOIN (SELECT s.groupid,count(*) count FROM  ".$this->server_set->get_table_name()." s group by s.groupid ) t ON sg.id=t.groupid LEFT JOIN (SELECT ss.groupid,count(*) count FROM  ".$this->member_set->get_table_name()." ss where ".($_SESSION['ADMIN_LEVEL']!=1 ? "ss.uid!=".$_SESSION['ADMIN_UID'] : "1")." group by ss.groupid ) tt ON sg.id=tt.groupid Where $where  ORDER BY convert(sg.groupname using gbk) ASC";
			}else{
					...
					if($user['ldap']){
						...
						for($ii=0; $ii<count($appsgroups); $ii++){
							...
							if($appsgroups[$ii]['ldapid']==0){
								...
								$sgroups[$ii]['count']+=$_child['count'];
							}
						}					
						$appsgroups=$this->allsgroup;
						$this->allsgroup=null;
					}
				}
				$appservers[$i]['appsgroups'] = $appsgroups;
				$psgroups = null;
				$psgroupsid = null;
				$appsgroups=null;
			}//var_dump($appsgroups);
			...
			if(	$_SESSION['ADMIN_LEVEL'] == 0) {
				...
				for($i=0;$i<$row_num;$i++) {
					...
					if($logintype=='apppub' || $logintype=='_apppub'){
						...
						$sql = "SELECT b.id appdeviceid, b.device_ip, c.appserverip,c.name,ap.icon,c.path,c.url FROM ".$this->appdevice_set->get_table_name()." b  LEFT JOIN ".$this->apppub_set->get_table_name()." c ON b.apppubid=c.id LEFT JOIN ".$this->appprogram_set->get_table_name()." ap ON c.appprogramname=ap.name ";
						if($gid){
							$sql .= " LEFT JOIN ".$this->server_set->get_table_name()." s ON b.device_ip=s.device_ip ";
						}
						$sql .= " WHERE (b.id IN(select appdeviceid FROM ".$this->appmember_set->get_table_name(). " WHERE memberid=".$_SESSION['ADMIN_UID'].") OR b.id IN(select appdevicesid FROM ".$this->appresgroup_set->get_table_name(). " WHERE appgroupname IN(select ag.appgroupname FROM ".$this->luser_appresourcegrp_set->get_table_name()." la LEFT JOIN ".$this->appresgroup_set->get_table_name()." ag ON la.appresourceid=ag.id   WHERE memberid=".$_SESSION['ADMIN_UID'].")) OR b.id IN(select distinct appdevices.id from ".$this->luser_appresourcegrp_set->get_table_name()." a left join (select a.id,b.groupid,b.username from ".$this->appresgroup_set->get_table_name()." a left join ".$this->appresgroup_set->get_table_name()." b on a.appgroupname=b.appgroupname where a.appdevicesid=0 and b.appdevicesid=-1 ) t on a.appresourceid=t.id left join servers s on s.groupid=t.groupid left join appdevices on appdevices.device_ip=s.device_ip where t.id and appdevices.id  and t.groupid and t.groupid=s.groupid AND IF(t.username='0', 1, t.username=appdevices.username) and a.memberid=".$_SESSION['ADMIN_UID'].")";
						if($member['groupid']){
						...
						if($gid){
							$_gidinfo = $this->sgroup_set->select_by_id($gid);
							if($_CONFIG['LDAP']){
								$sql .= "  AND (s.groupid='".intval($gid)."' or groupid IN(".$_gidinfo['child']."))";
							}else{
								$sql .= "  AND s.groupid='".intval($gid)."'";
							}
	...
	function do_devices_cache(){
		...
		$this->server_set->query("INSERT INTO devices_group_cache(memberid, groupid, groupname, ct, ldapid) SELECT ".$_SESSION['ADMIN_UID'].",sg.id, sg.groupname,t.sct,sg.ldapid FROM servergroup sg LEFT JOIN (SELECT s.groupid,count(*) sct FROM devices d LEFT JOIN (SELECT devicesid FROM luser WHERE memberid=".$_SESSION['ADMIN_UID']." UNION SELECT devicesid FROM lgroup WHERE groupid=".intval($_SESSION['ADMIN_GROUP'])." UNION SELECT devicesid FROM resourcegroup WHERE groupname IN (SELECT b.groupname FROM luser_resourcegrp a LEFT JOIN resourcegroup b ON a.resourceid=b.id WHERE a.memberid=".$_SESSION['ADMIN_UID'].") UNION SELECT devicesid FROM resourcegroup WHERE groupname IN (SELECT b.groupname FROM lgroup_resourcegrp a LEFT JOIN resourcegroup b ON a.resourceid=b.id WHERE a.groupid=".intval($_SESSION['ADMIN_GROUP']).")) dd ON d.id=dd.devicesid LEFT JOIN servers s ON d.device_ip=s.device_ip WHERE dd.devicesid IS NOT NULL and d.login_method!=26 group by s.groupid ) t ON sg.id=t.groupid Where 1=1 AND t.sct > 0 ORDER BY sg.groupname ASC");
...
$_SESSION['CHKSMSLOGIN_STEP']=null;
	...
	function chklogin() {
		...
		if(empty($result)){
			...
			$this->loginacct_set->add($loginacct);
			alert_and_back('该用户已经锁定');
			exit;
		}elseif(empty($result[0][$authtype])&&!($authtype=='localauth'&&$_SESSION['LOGIN_RADIUS_TO_LOCAL'])){
		...
		if($result[0]['cacn']&&intval($_POST['memberscount'])==0&&$_CONFIG['Certificate']){
			...
			$this->loginacct_set->add($loginacct);
			alert_and_back($result[0]['username'].'已经绑定的证书不匹配');
			exit;
		}
		...
		//if($result[0]['loginlock'])
		{
			if($result[0]['logintimes'] >= $login_times){
				...
				if($lastdate_add_1_day < time()){
					...
					$this->loginacct_set->add($loginacct);
					alert_and_back(language("您登录次数达到了最大次数").":".$login_times.','.language('请'.$pwdconfig['login_times_last'].'分钟后再试'));
					exit;
				}
			}else{
		...
		if($authtype=='localauth'){
			...
			if(intval($_SESSION['CHKSMSLOGIN_STEP'])<1&&!$can_login) 
			{
				...
				$cologinauth = (($result[0]['usbkey']&&$result[0]['usbkeystatus']==11) || $result[0]['authtype']&&($result[0]['radiusauth']||$result[0]['ldapauth']||$result[0]['adauth']||intval($result[0]['auth'])==2));
					...
					$newmember->set_data("webportallogin", (empty($result[0]['webportaltime']) ? mktime(23,59,59,date('n'),date('j'),date('Y')) : mktime()+$result[0]['webportaltime']*60));
				...
				if($username=='admin'){
					...
					if(empty($out[0]) || ($out[0]&&(trim($out[0])==-1 || date('Y-m-d')>substr($out[0], 0, strpos($out[0], ' '))))){
						alert_and_back('系统Licenses错误','admin.php');
					}
				...
				default:
				{
					if(/*$result[0]['level']==0 || $_CONFIG['OTHER_MEMBER_RADIUS']==1*/1){
						$_SESSION['LOGIN_RADIUS_TO_LOCAL']=1;
							...
							$this->loginacct_set->add($loginacct);
						alert_and_back('系统错误,请稍候重试,或用本地方式登录,或联系管理员');
						exit;
					}
				}
			}
			...
			$auth_rs = $this->ldap_chk($username, $password, $ldapconfig['address'], $ldapconfig['port'],$ldapconfig['domain']);
			//$auth_rs=1;
	...
	function doqrcode(){
		...
		if($minfo['authtype']&&($minfo['localauth']||$minfo['radiusauth']||$minfo['ldapauth']||$minfo['adauth']||intval($minfo['auth'])==2)){
			...
			if($minfo['ldapauth']&&$_SESSION['authtype']!='ldapauth'){
				...
				$ldapauth = $this->ldap_chk($minfo['username'], $ldappwd, $ldapconfig['address'], $ldapconfig['port'],$ldapconfig['domain']);
			}
		...
		$m = new member();
		$m->set_data("webportallogin", (empty($minfo['webportaltime']) ? mktime(23,59,59,date('n'),date('j'),date('Y')) : mktime()+$minfo['webportaltime']*60));
	...
	function create_license(){
		...
		if(!empty($lines))
		{
			...
			for($ii=0; $ii<count($lines); $ii++)
			{
				...
				if(substr(trim($lines[$ii]),0,1)!='#'&&strstr($lines[$ii], "LicensesDevice"))
				{
					$lines[$ii] = "LicensesDevice\t\t\t\t\t\t\t$device\n";
					break;
				}
			}
		}
		...
		{
			alert_and_close($filename.'配置文件不存在或没有权限');
			exit;
		}
...
//echo "/opt/freesvr/audit/sbin/collect -i $device";
		...
		exec("/opt/freesvr/audit/sbin/collect -i $device",$out);
		echo '<div style="word-wrap:break-word; width:220px;border:solid 1px #ccc;">'.$out[0].'</div>';
		exit;
	}
