...
//		echo '<pre>';print_r($resources);echo '</pre>';echo '<pre>';print_r($res);echo '</pre>';
	...
	function app_group_save() {
		...
		for($i=0; $i<count($selected); $i++){
			...
			if($selected_arr[0]=='groupid'){
				...
				$resourcegroup->set_data('appdevicesid',-1);
		...
		if($sgroup_arr){
			for($i=0; $i<count($sgroup_arr); $i++){
				$sql = "select d.id from appdevices d left join servers s on d.device_ip=s.device_ip left join servergroup sg on s.groupid=sg.id where sg.id=".$sgroup_arr[$i][0]."";
				if($sgroup_arr[$i][1]!=0){
		...
		$this->appresourcegroup_bindsave($id);
	...
	function appresourcegroup_bindsave($id=null){
		$sid = get_request("id");
		if(empty($sid)) $sid=$id;
		$sessionluser = get_request('sessionluser', 1, 1);
		$sessionlgroup = get_request('sessionlgroup', 1, 1);
