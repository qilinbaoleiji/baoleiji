...
class c_admin_apppub extends c_base {
	function index($where = NULL) {
		...
		if($delete) {
			...
			$row_num = $this->appcomm_set->base_select("SELECT count(*) ct FROM ".$this->appcomm_set->get_table_name()." a LEFT JOIN ".$this->member_set->get_table_name()." m ON a.memberid=m.uid LEFT JOIN ".$this->rdp_set->get_table_name()." d ON a.sid=d.sid $where and a.sid > 0");
			$row_num = $row_num[0]['ct']; //LEFT JOIN ".$this->plsqlhistory_set->get_table_name()." e ON a.id=e.appcommids
			$newpager = new my_pager($row_num, $page_num, $this->config['site']['items_per_page'], 'page');
			$allsession=$this->appcomm_set->base_select("SELECT a.*,m.username,m.realname,d.cli_addr FROM ".$this->appcomm_set->get_table_name()." a LEFT JOIN ".$this->member_set->get_table_name()." m ON a.memberid=m.uid LEFT JOIN ".$this->rdp_set->get_table_name()." d ON a.sid=d.sid $where and a.sid > 0  ORDER BY $orderby1 $orderby2  LIMIT $newpager->intStartPosition, $newpager->intItemsPerPage ");
	...
	function view() {
		...
		$row_num = $row_num[0]['ct']; //LEFT JOIN ".$this->plsqlhistory_set->get_table_name()." e ON a.id=e.appcommids
		$newpager = new my_pager($row_num, $page_num, $this->config['site']['items_per_page'], 'page');
		$allcommand=$this->appcomm_set->base_select("SELECT a.*,m.username,m.realname,d.cli_addr FROM ".$this->appcomm_set->get_table_name()." a LEFT JOIN ".$this->member_set->get_table_name()." m ON a.memberid=m.uid LEFT JOIN ".$this->rdp_set->get_table_name()." d ON a.sid=d.sid $where and a.sid > 0  ORDER BY $orderby1 $orderby2  LIMIT $newpager->intStartPosition, $newpager->intItemsPerPage ");
