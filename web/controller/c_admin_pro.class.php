...
class c_admin_pro extends c_base {
	function dev_index($interface=false) {
		...
		$sql = "SELECT servers.*,(UNIX_TIMESTAMP(servers.asset_warrantdate)-UNIX_TIMESTAMP())/(60*60*24) warrantdays,dd.userct,dda.appuserct,ddapp.appct,
	...
	function groupadddev_save(){
		...
		echo '<script>window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=dev_server&gid='.$gid.'";</script>';
			//exit;
	...
	function dev_edit() {
		...
		require('./include/select_sgroup.inc.php');
	...
	function dev_save() {
		...
		*/
		...
		for($i=1; true; $i++){
			if($_POST['groupid'.$i]){
				$_SESSION['ADDDEV']['groupid']=$new_group = $_POST['groupid'.$i];
				continue;
			}			
			break;
		}
		...
		if(!ctype_digit($new_sshport)|| $new_sshport <1 || $new_sshport>65534 ){
			alert_and_back("SSH端口请输入1-65534");
			return;
		}
		if(!ctype_digit($new_telnetport)|| $new_telnetport <1 || $new_telnetport>65534 ){
			alert_and_back("TELNET端口请输入1-65534");
			return;
		}
		if(!ctype_digit($new_ftpport)|| $new_ftpport <1 || $new_ftpport>65534 ){
			alert_and_back("FTP端口请输入1-65534");
			return;
		}
		if(!ctype_digit($new_rdpport)|| $new_rdpport <1 || $new_rdpport>65534 ){
			alert_and_back("RDP端口请输入1-65534");
			return;
		}
		if(!ctype_digit($new_vncport)|| $new_vncport <1 || $new_vncport>65534 ){
			alert_and_back("VNC端口请输入1-65534");
			return;
		}
		if(!ctype_digit($new_x11port)|| $new_x11port <1 || $new_x11port>65534 ){
			alert_and_back("X11端口请输入1-65534");
			return;
		}
		if(!is_ip($new_ip)){
	...
	function devbatchadd() {		
		...
		require_once('./include/select_sgroup.inc.php');
	...
	function batchdevsave(){
		...
		if($succeed){
			...
			echo '<script>alert("'.$msg.'");window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=dev_server&gid='.$gid.'";</script>';
	...
	function dev_del() {
		...
		for($i=0; $i<count($id_arr); $i++){
			...
			if(	$this->appdevice_set->select_count("device_ip = '".$old_dev['device_ip']."'") > 0) {
				...
				continue;
				//echo '<script>alert("删除成功");window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=dev_server&gid='.$gid.'";</script>';
				//exit;
...
*/
	...
	function insertrandom($ip, $device_ip, $username, $luser, $time, $code){
		global $dbcharset, $_CONFIG, $dbuser, $dbpwd;
		if($_CONFIG['RANDOM_DB']=='0'){
			$ip="localhost";
		}
		$lblink = @mysql_connect($ip, $dbuser, $dbpwd);
		if(!$lblink){
			return false;
		}
		...
		$sql = "INSERT INTO random SET device_ip='".$device_ip."', username='".$username."', luser='".$luser."', time='".$time."', code='".$code."', inputusername='".$_SESSION['LOGIN_USERNAME']."', inputpassword=udf_encrypt('".$_SESSION['LOGIN_PASSWORD']."'), logincommit='".intval($_SESSION['COMMITID'])."'";
	...
	function dotwoauth(){
		...
		//echo '<script>window.opener.location.href="'.$url.'";window.close();</script>';
		echo '<script>window.parent.loadurl("'.$url.'");'.'window.parent.closeWindow();</script>';
		exit;
	}
	...
	function dev_login() {
		...
		if(strtolower($tem['login_method']) == 'vnc'){
			if(!in_array($id,$_SESSION['EDITVNCPORTDEVICESIDS'])){
				if($rdptype=='activex'){
					...
					echo "window.loadurl('admin.php?controller=admin_pro&action=editvncport&".mktime()."')";
				}
				exit;
			}
		}
		if($dev['logincommit']){
			if(!in_array($id,$_SESSION['COMMITDEVICESIDS'])){
				if((strtolower($tem['login_method']) == 'rdp'||strtolower($tem['login_method']) == 'vnc')&&$rdptype=='activex'){
					...
					echo "window.loadurl('admin.php?controller=admin_pro&action=showcommit&".mktime()."')";
				}
		...
		if($dev['entrust_username']==0){
			if(!in_array($id,$_SESSION['INPUTDEVICESIDS'])){
				...
				$_continue = ($blankuser || (strtolower($tem['login_method']) == 'ftp'||(strtolower($tem['login_method']) == 'ssh'&&($logintool=='winscp'||$logintool=='flashxp'))));
				...
				if($_continue){
					if((strtolower($tem['login_method']) == 'rdp'||strtolower($tem['login_method']) == 'vnc')&&$rdptype=='activex'){
						...
						echo "window.loadurl('admin.php?controller=admin_pro&action=showinputauth&".mktime()."')";
					}
		...
		if($_row[0]['smsalert']){
			if(!in_array($id,$_SESSION['SMSAUTHDEVICESIDS'])){
				echo "window.loadurl('admin.php?controller=admin_pro&action=showsmsauth&".mktime()."')";
				exit;
			}
		}
	...
	function dev_group($interface=false) {
		...
		if($_CONFIG['LDAP']){
			...
			$newpager = new my_pager($row_num, $page_num, $this->config['site']['items_per_page'], 'page');
			$alldev = $this->sgroup_set->base_select("SELECT * FROM ".$this->sgroup_set->get_table_name()." WHERE ".$_where.' AND '.$where. " ORDER BY $orderby1 $orderby2 LIMIT $newpager->intStartPosition, $newpager->intItemsPerPage");
			for($i=0; $i<count($alldev); $i++){
				if($interface){
					$subchild = $this->get_devgroup_child_for_interface($alldev[$i], 2, $_groupid, $i);
					$alldev[$i]['childs']=$subchild;
				}else{
					$subchild = $this->get_devgroup_child($alldev[$i], 2, $_groupid, $i);
					$alldev[$i]['groupstr']=$subchild['groupstr'];
				}
					$alldev[$i]['count']=$subchild['count'];
					$alldev[$i]['mcount']=$subchild['mcount'];
					$alldev[$i]['childrenct']=$subchild['childrenct'];
			}
	...
	function resourcegroup_bind($fromedit=null){
		...
		if($ori_g_id){
			...
			$_tmpgid = $this->sgroup_set->select_all("id IN(".implode(',', $ori_g_id).")");
			$__tmpgid = $_tmpgid['child'];
	...
	function resourcegroup_bindsave($id=null){
		$sid = get_request("id");
		if(empty($sid)) $sid=$id;
		$sessionluser = get_request('sessionluser', 1, 1);
		$sessionlgroup = get_request('sessionlgroup', 1, 1);
	...
	function dev_group_save() {
		...
		for($i=1; true; $i++){
			if($_POST['groupid'.$i]){
				$selectedldapid = $_POST['groupid'.$i];
				continue;
			}			
			break;
		}
		...
		//exit;
		if($_POST['groupid1']){
			$exists_count = $this->sgroup_set->select_by_id($_POST['groupid1']);
		...
		if(0==$exists_count) {
			...
			if($id){
				...
				if($levelx!=$old['level']){
					if($levelx==0&&$this->sgroup_set->select_count("ldapid=".$old['id'])>0){
						alert_and_back('该目录下已有目录或资源组，请先删除');
						return;
					}elseif($levelx!=0){
						$diff=($levelx-$old['level'])>0 ? '+'.($levelx-$old['level']) : '-'.($old['level']-$levelx);
						if($this->sgroup_set->base_select("SELECT id FROM ".$this->sgroup_set->get_table_name()." WHERE ldapid=".$old['id']." HAVING MAX(level)".$diff.">5")){
							alert_and_back('该目录降级后超过系统最大分级数5');
							exit;
						}
						$update_sql = "UPDATE ".$this->sgroup_set->get_table_name()." SET level=level".$diff." WHERE id IN(".$old['child'].") AND level>0 AND id!=".$old['id'];
					}
				}
				$new_group->set_data('id',$id);
				$this->sgroup_set->edit($new_group);
				if(mysql_affected_rows()>0&&!empty($update_sql)){
					$this->sgroup_set->query($update_sql);					
				}
				
				$adminlog = new admin_log();	
				$adminlog->set_data('administrator', $_SESSION['ADMIN_USERNAME']);			
				$adminlog->set_data('action', language('编辑资源组'));
				$adminlog->set_data('resource', '设备'.($levex ? '目录' : '组').':'.$new_name);
				$this->admin_log_set->add($adminlog);
	...
	function dev_group_del() {
		...
		if(	 0 == $this->server_set->select_count("groupid IN(".$old_group['child'].") ") && 0 == $this->member_set->select_count("groupid IN(".$old_group['child'].") ") ) {
			$gid = $this->sgroup_set->select_all("id='$id' ".($_CONFIG['LDAP'] ? "or id IN(".$old_group['child'].") " : ''));
			for($i=0; $i<count($gid); $i++){
				$groupinfo = $this->sgroup_set->select_by_id($gid[$i]['id']);
				$adminlog = new admin_log();	
				$adminlog->set_data('administrator', $_SESSION['ADMIN_USERNAME']);			
				$adminlog->set_data('action', language('删除资源组'));
				$adminlog->set_data('resource', '设备'.($groupinfo['level'] ? '目录' : '组').':'.$groupinfo['groupname']);
				$this->admin_log_set->add($adminlog);
				$this->sgroup_set->delete($gid[$i]['id']);
			}
			$this->server_set->delete_all("groupid='$id' or groupid IN(".$old_group['child'].") ");
			$this->member_set->delete_all("groupid='$id' or groupid IN(".$old_group['child'].") ");
			if($from=='usergroup'){
	...
	function groupbatchsave(){
		...
		for($i=0; $i<20; $i++){
			...
			if($_POST['groupid'.$i.'1']){
				$exists_count = $this->sgroup_set->select_by_id($_POST['groupid'.$i.'1']);
			...
			$new_group->set_data('ldapid',$_POST['groupid'.$i.'1']);
			$this->sgroup_set->add($new_group);
		}
		$msg='操作成功';
		if($error){
	...
	function dev_delfromgroup(){
		...
		echo '<script>window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=dev_server&gid='.$dev['groupid'].'";</script>';
			//exit;
	...
	function pass_edit() {
		...
		for($i=0; $i<count($_SESSION[$sessionluser]); $i++){
			$bindedsshkey = $this->sshkey_set->select_all("memberid='".$_SESSION[$sessionluser][$i]['memberid']."' AND devicesid='".$_SESSION[$sessionluser][$i]['devicesid']."'");
			$_SESSION[$sessionluser][$i]['sshkeyname'] = $bindedsshkey[0]['sshkeyname'];
			$aclpolicy = $this->restrictpolicy_set->select_all("memberid='".$_SESSION[$sessionluser][$i]['memberid']."' AND devicesid='".$_SESSION[$sessionluser][$i]['devicesid']."'");
			if($aclpolicy){
				$_SESSION[$sessionluser][$i]['restrictacl']=$aclpolicy[0]['aclid'];
			}
		...
		for($i=0; $i<count($_SESSION[$sessionlgroup]); $i++){
			$aclpolicy = $this->restrictpolicy_set->select_all("usergroupid='".$_SESSION[$sessionlgroup][$i]['groupid']."' AND devicesid='".$_SESSION[$sessionlgroup][$i]['devicesid']."'");
			if($aclpolicy){
				$_SESSION[$sessionlgroup][$i]['restrictacl']=$aclpolicy[0]['aclid'];
			}
		...
		if($id != 0) {
			$sql = "select distinct member.username from lgroup left join member on lgroup.groupid=member.groupid where lgroup.devicesid=$id and member.uid union select distinct member.username from luser_resourcegrp a left join (select a.id from resourcegroup a left join resourcegroup b on a.groupname=b.groupname where a.devicesid=0 and b.devicesid=$id ) t on a.resourceid=t.id left join member on a.memberid=member.uid where t.id and member.uid union select distinct member.username from lgroup_resourcegrp a left join (select a.id from resourcegroup a left join resourcegroup b on a.groupname=b.groupname where a.devicesid=0 and b.devicesid=$id ) t on a.resourceid=t.id left join member on a.groupid=member.groupid where t.id and member.uid ";
			$allmem = $this->member_set->base_select("SELECT a.*,IF(t.username IS NULL, 0, 1) binded FROM member a LEFT JOIN ($sql)t ON a.username=t.username WHERE level!=11 ".(empty($webuser) ? '' : " AND a.username like '%$webuser%' " )." ".$wheremember." AND uid not in (SELECT radiususer FROM ".$this->devpass_set->get_table_name()." WHERE radiususer>0)".($_SESSION['ADMIN_LEVEL']==3 || $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ' AND groupid IN('.implode(',', $_SESSION['ADMIN_MUSERGROUP_IDS']).') ' : '')." ORDER BY a.username ASC");
			$this->assign('id',$id);
			...
			$this->assign('ipv6enable', $old_pass['ipv6enable']);
			$this->assign('key_input', $old_pass['key_input']);
			$this->assign('fastpath_input', $old_pass['fastpath_input']);
			$this->assign('fastpath_output', $old_pass['fastpath_output']);
			$this->assign('su_passwd', $old_pass['su_passwd']);
			...
			$this->assign('dipv6enable',$dp['ipv6enable']);
		}
...
//echo '<pre>';print_r($_SESSION);echo '</pre>';
	...
	function pass_save() {
		...
		$new_password = htmlspecialchars_decode(get_request('password',1,1));
		$new_password_confirm = htmlspecialchars_decode(get_request('password_confirm',1,1));
		...
		$key_input = get_request('key_input', 1, 1);
		$fastpath_input = get_request('fastpath_input', 1, 1);
		$fastpath_output = get_request('fastpath_output', 1, 1);
		$su_passwd = get_request('su_passwd', 1, 1);
		...
		if($id == 0) {
			if(0 == $this->devpass_set->select_count("username = '$new_name' and device_ip='$ip' and port='$port' and hostname='".$server['hostname']."' and login_method='$login_method'")) {
				...
				$new_pass->set_data('ipv6',$server['ipv6']);
				...
				if($ipv6enable == 'on') {
					$new_pass->set_data('ipv6enable',1);
				}
				else {
					$new_pass->set_data('ipv6enable',0);
				}
				if($key_input == 'on') {
					$new_pass->set_data('key_input',1);
				}
				else {
					$new_pass->set_data('key_input',0);
				}
				if($fastpath_input == 'on') {
					$new_pass->set_data('fastpath_input',1);
				}
				else {
					$new_pass->set_data('fastpath_input',0);
				}
				if($fastpath_output == 'on') {
					$new_pass->set_data('fastpath_output',1);
				}
				else {
					$new_pass->set_data('fastpath_output',0);
				}
				if($su_passwd == 'on') {
					$new_pass->set_data('su_passwd',1);
				}
				else {
					$new_pass->set_data('su_passwd',0);
				}
				if($radiususer == 'on') {
	...
	function export_pass($where='1=1'){
		...
		$str = language("主机名").",".language("IP").",".($_CONFIG['LDAP']&&0 ? language("一级目录").",".language("二级目录")."," : '').language("服务器组").",".language("系统类型").",".language("系统用户").",".language("登录方式")." ,".language("当前密码")." ,".language("端口")." ,".language("过期时间")." ,".language("自动修改密码")." ,".language("主账号")." ,".language("自动登录")." ,".language("堡垒机用户认证")." ,".language("SFTP")." ,".language("公私钥认证")."\n";
		...
		if($alldev){
			...
			foreach ($alldev AS $report){	
				...
				$str .= $report['hostname'].",".$report['device_ip'].",".($_CONFIG['LDAP']&&0 ? $report['sgroupparentparent'].",".$report['sgroupparent']."," : '').$report['groupname'].",".$report['device_type'].",".($report['username']==""  ? '空用户' : $report['username']).",".$report['login_method'].",".$report['cur_password'].",".$report['port'].",".($report['limit_time']=='9999-00-00' ? '永不过期' : $report['limit_time']).",".($report['automodify']? '是':'否').",".($report['master_user']? '是':'否').",".($report['entrust_password']? '是':'否').",".$report['radiususer'].",".($report['sftp']? '是':'否').",".($report['publickey_auth']? '是':'否');
				$str .= "\n";
			}
		}
	...
	function dodevimport(){
		...
		for($i=1; $i<count($lines); $i++){
			...
			$index_i++;
			$login_method=trim($linearr[$index_i++]);
			$current_password=trim($linearr[$index_i++]);
			$port=intval($linearr[$index_i++]);
			$limit_time=trim($linearr[$index_i++]);
			$auto=trim($linearr[$index_i++]);
			$master_user=trim($linearr[$index_i++]);
			$entrust_password=trim($linearr[$index_i++]);
			$radiususer=trim($linearr[$index_i++]);
			$sftp=trim(($linearr[$index_i++]=='是' ? 1 : 0 ));
			$publickey_auth=trim(($linearr[$index_i++]=='是' ? 1 : 0 ));
			...
			if(empty($devpass)){
				...
				$new_pass->set_data('cur_password',addslashes($current_password));
				if($encrypt){
		...
		if($error){
			...
			echo '<script>alert("导入成功");window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=dev_server&gid='.$gid.'";</script>';
		}
	...
	function showpasswordkey(){
		$id = get_request('id');
		if($_POST['submit']){
			if($this->member_set->select_count("username='password' AND password=udf_encrypt('".$_POST['password']."')")>0&&$this->member_set->select_count("username='audit' AND password=udf_encrypt('".$_POST['audit']."')")>0){
	...
	function lac_save(){
		foreach($_POST AS $key => $v){
			...
			if(substr($key, 0, 5)=='save_'&&count($tmp)>1){
				$sql = "UPDATE ".$this->devpass_set->get_table_name()." SET new_password=udf_encrypt('".$_POST['new_password_'.$tmp[1]]."'), active_change='".($_POST['active_change_'.$tmp[1]])."' WHERE id='".$_POST['id_'.$tmp[1]]."'";
...
<?php  
   ...
   sleep(2);
	...
	function dev_priority_search(){
		...
		$sql  = "select distinct * from (select distinct member.uid,member.username webuser,member.realname webrealname,member.groupid,member.lastdate,luser.devicesid,devices.device_ip,devices.username,devices.login_method,devices.device_type,luser.forbidden_commands_groups,luser.weektime,luser.sourceip,luser.autosu,luser.syslogalert,luser.mailalert,luser.loginlock from luser left join member on luser.memberid=member.uid left join devices on luser.devicesid=devices.id where member.uid and luser.devicesid AND $where";		
		$sql .= " union select distinct member.uid,member.username webuser,member.realname webrealname,member.groupid,member.lastdate,t.devicesid,devices.device_ip,devices.username,devices.login_method,devices.device_type,a.forbidden_commands_groups,a.weektime,a.sourceip,a.autosu,a.syslogalert,a.mailalert,a.loginlock from luser_resourcegrp a left join (select a.id,b.devicesid from resourcegroup a left join resourcegroup b on a.groupname=b.groupname where a.devicesid=0 and b.devicesid>0 ) t on a.resourceid=t.id left join member on a.memberid=member.uid left join devices on t.devicesid=devices.id where t.id and member.uid and t.devicesid AND $where";

		$sql .= " union select distinct member.uid,member.username webuser,member.realname webrealname,member.groupid,member.lastdate,devices.id devicesid,devices.device_ip,devices.username,devices.login_method,devices.device_type,a.forbidden_commands_groups,a.weektime,a.sourceip,a.autosu,a.syslogalert,a.mailalert,a.loginlock from luser_resourcegrp a left join (select a.id,b.groupid,b.username from resourcegroup a left join resourcegroup b on a.groupname=b.groupname where a.devicesid=0 and b.devicesid=-1 ) t on a.resourceid=t.id left join member on a.memberid=member.uid left join servers s on s.groupid=t.groupid left join devices on devices.device_ip=s.device_ip where t.id and member.uid and t.groupid and t.groupid=s.groupid AND IF(t.username='0', 1, t.username=devices.username) and devices.id AND $where";

		$sql .= " union select distinct member.uid,member.username webuser,member.realname webrealname,member.groupid,member.lastdate,lgroup.devicesid,devices.device_ip,devices.username,devices.login_method,devices.device_type,lgroup.forbidden_commands_groups,lgroup.weektime,lgroup.sourceip,lgroup.autosu,lgroup.syslogalert,lgroup.mailalert,lgroup.loginlock from lgroup left join member on lgroup.groupid=member.groupid left join devices on lgroup.devicesid=devices.id where member.uid and lgroup.devicesid AND $where";
		...
		if($derive==1){
			...
			$str = language("序号")."\t".(($type == 'luser') ? '运维账号'."\t".'别名'."\t运维组" : '运维组')."\t".language("设备IP")."\t".language("系统用户")." \t".language("黑名单")." \t".language("周组策略 ")." \t".language("来源IP")." \t".language("自动为超级用户")." \t".language("SYSLOG告警")." \t".language("Mail告警")." \t".language("账号锁定")."\t".(($type == 'luser') ? '上次登录'." \t" : '').language("协议")."\n";
			$id=1;
			if($reports)
			foreach ($reports AS $report){
				$str .= ($id++)."\t".(($type == 'luser') ? $report['webuser']."\t".$report['webrealname']."\t".$report['usergroup'] :$report['gname'])."\t".$report['device_ip'].'('.$report['hostname'].')'."\t".$report['username']."(".$report['realname'].")\t".$report['forbidden_commands_groups']."\t".$report['policyname']."\t".$report['sourceip']."\t".($report['autosu'] ? '是' : '否')."\t".($report['syslogalert'] ? '是' : '否')."\t".($report['mailalert'] ? '是' : '否')."\t".($report['loginlock'] ? '是' : '否')."\t".(($type == 'luser') ? $report['lastdate']." \t" : '').$report['login_method'];
				$str .= "\n";
			}
			Header('Cache-Control: private, must-revalidate, max-age=0');
			Header("Content-type: application/octet-stream"); 
	...
	function systempriority_data($alldev){
		...
		for($i=0; $i<$num; $i++){
			...
			$alldev[$i]['sgroupid'] = $servergroup[0]['groupid'];
			$alldev[$i]['hostname'] = $servergroup[0]['hostname'];
			$alldev[$i]['month'] = $servergroup[0]['month'];
			$alldev[$i]['week'] = $servergroup[0]['week'];
			$alldev[$i]['user_define'] = $servergroup[0]['user_define'];
			if($allgroup){
				foreach($allgroup as $groupt){
					if($groupt[id]==$alldev[$i][sgroupid]){//var_dump($groupt);
						$alldev[$i][groupname]=$groupt[groupname];
						if($groupt[ldapid]){
	...
	function resource_group_edit() {
		...
		$res = $this->devpass_set->base_select("SELECT d.*,l.login_method lmname,if(entrust_password=1,'托密','不托密') as ep FROM devices d LEFT JOIN (SELECT distinct devicesid FROM ".$this->resgroup_set->get_table_name()." WHERE groupname='".$ginfo['groupname']."' and devicesid!=0) t ON d.id=t.devicesid LEFT JOIN login_template l ON d.login_method=l.id    LEFT JOIN (SELECT * FROM servers WHERE 1 ".($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ($_SESSION['ADMIN_MSERVERGROUP']==0 ? ' AND 0' : '').' AND (groupid IN('.implode(',', $_SESSION['ADMIN_MSERVERGROUP_IDS']).'))' : '').") s ON d.device_ip=s.device_ip  WHERE 1 AND t.devicesid IS NOT NULL AND s.id IS NOT NULL "." ORDER BY device_ip");

		$res2 = $this->devpass_set->base_select("SELECT a.groupid,a.username,b.groupname FROM ".$this->resgroup_set->get_table_name()." a left join ".$this->sgroup_set->get_table_name()." b ON a.groupid=b.id WHERE a.groupname='".$ginfo['groupname']."' and devicesid=-1 ".($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ($_SESSION['ADMIN_MSERVERGROUP']==0 ? ' AND 0' : '').' AND (groupid IN('.implode(',', $_SESSION['ADMIN_MSERVERGROUP_IDS']).'))' : '')." ORDER BY convert(b.groupname using gbk) ASC");
		$res2_groupid = array(-1);
		$res2_deviceid = array(-1);
		for($i=0; $i<count($res2); $i++){
			if(empty($res2[$i]['groupid'])) continue;
			if($res2[$i]['username']=='0'){
				$res2_groupid[]=$res2[$i]['groupid'];
			}else{				
				$_tmp = $this->devpass_set->base_select("SELECT d.id FROM devices d LEFT JOIN servers s ON d.device_ip=s.device_ip  WHERE 1 AND d.id IS NOT NULL AND s.id IS NOT NULL AND groupid=".$res2[$i]['groupid']." AND d.username='".$res2[$i]['username']."'");
				for($j=0; $j<count($_tmp); $j++){
					$res2_deviceid[]=$_tmp[$j]['id'];
				}
		...
		$resources = $this->devpass_set->base_select("SELECT d.*,l.login_method lmname,if(entrust_password=1,'托密','不托密') as ep FROM devices d LEFT JOIN (SELECT distinct devicesid FROM ".$this->resgroup_set->get_table_name()." WHERE groupname='".$ginfo['groupname']."' and devicesid!=0) t ON d.id=t.devicesid LEFT JOIN login_template l ON d.login_method=l.id LEFT JOIN (SELECT * FROM servers WHERE groupid not in(".implode(',', $res2_groupid).") ".($_SESSION['ADMIN_LEVEL']==3|| $_SESSION['ADMIN_LEVEL']==21 || $_SESSION['ADMIN_LEVEL']==101 ? ($_SESSION['ADMIN_MSERVERGROUP']==0 ? ' AND 0' : '').' AND (groupid IN('.implode(',', $_SESSION['ADMIN_MSERVERGROUP_IDS']).'))' : '').")s ON d.device_ip=s.device_ip  WHERE $where AND t.devicesid IS NULL AND s.id IS NOT NULL AND d.id not in(".implode(',', $res2_deviceid).")"." ORDER BY device_ip");
	...
	function resource_group_save() {
		...
		if($gname==""){
			...
			$gid = mysql_insert_id();
			for($i=0; $i<count($_SESSION[$sessionluser]); $i++){
				$_SESSION[$sessionluser][$i]['resourceid']=$gid;
			}
			for($i=0; $i<count($_SESSION[$sessionlgroup]); $i++){
				$_SESSION[$sessionlgroup][$i]['resourceid']=$gid;
			}
			unset($resourcegroup);
		}
		if($gid){
		...
		for($i=0; $i<count($selected); $i++){
			...
			if($selected_arr[0]=='groupid'){
				$sgroup_arr[]=array($selected_arr[1],$selected_arr[2]) ;
				$resourcegroup->set_data('devicesid',-1);
				$resourcegroup->set_data('groupid',$selected_arr[1]);
				$resourcegroup->set_data('username','0');
			}else{
		...
		if($sgroup_arr){
			for($i=0; $i<count($sgroup_arr); $i++){
				$sql = "select d.id from devices d left join servers s on d.device_ip=s.device_ip left join servergroup sg on s.groupid=sg.id where sg.id=".$sgroup_arr[$i][0]."";
				if($sgroup_arr[$i][1]!=0){
					$sql .= " AND d.username='".$sgroup_arr[$i][1]."'";
				}
				$this->resgroup_set->query("delete from ".$this->resgroup_set->get_table_name()." where devicesid IN(".$sql.") and groupname='".$gname."'");
				if(mysql_affected_rows()>0){
					$msg = "，并合并重复的权限项";
				}
			}
		}
		$this->resourcegroup_bindsave($gid);
		alert_and_back("操作成功".$msg,'admin.php?controller=admin_pro&action=resource_group_edit&id='.$gid.'&back=1');
		return $gid;
	}
...
//echo '<pre>';print_r($_SESSION);echo '</pre>';
	...
	function sshkey_doimport(){
		...
		if(is_uploaded_file($_FILES['sshkey']['tmp_name'])&&substr($_FILES['sshkey']['name'], strrpos($_FILES['sshkey']['name'], '.'))=='.zip'){
			...
			for($i=0; $i<count($lines); $i++){
				...
				$sshprivatekey=$_CONFIG['PASSEDITSSHPULICKEY_NEW'].'/'.$lines[$i][2];
				...
				if($exist_name){
					...
					if($lines[$i][2]&&!copy('tmp/pvt/'.$lines[$i][2], $_CONFIG['PASSEDITSSHPULICKEY_NEW'].'/'.$lines[$i][2])){
						$err[]='文件'.$lines[$i][2].'移动失败，请重新上传\n';
						continue;
					}				
					if($lines[$i][2]&&@file_exists($_CONFIG['PASSEDITSSHPULICKEY_NEW'].'/'.$lines[$i][2])){
						@exec("chmod 600 ".$_CONFIG['PASSEDITSSHPULICKEY_NEW'].$lines[$i][2]);
					}
					...
					if($lines[$i][2])
					$skname->set_data('sshprivatekey', $_CONFIG['PASSEDITSSHPULICKEY_NEW'].'/'.$lines[$i][2]);
	...
	function sshkey_new(){
		...
		$row_num = $row_num[0]['row_num'];
		$newpager = new my_pager($row_num, $page_num, $this->config['site']['items_per_page'], 'page');
		$sshdevices = $this->sshkey_set->base_select("SELECT a.*,m.username webuser,d.device_ip,d.port,d.username FROM ".$this->sshkey_set->get_table_name()." a left join ".$this->member_set->get_table_name()." m ON a.memberid=m.uid left join ".$this->devpass_set->get_table_name()." d ON a.devicesid=d.id WHERE $where ORDER BY device_ip ASC LIMIT ".$newpager->intStartPosition.", ".$newpager->intItemsPerPage);
		$curr_url = $_SERVER['PHP_SELF'] . "?";
		if(strstr($_SERVER['QUERY_STRING'], "&page=")) {
			$curr_url .= substr($_SERVER['QUERY_STRING'], 0 , strpos($_SERVER['QUERY_STRING'], "&page="));
		}
		...
		for($i=0; $i<count($_POST['id']); $i++){
			...
			if(in_array($s['ip'], $_POST['ips'])){
				...
				if($this->server_set->select_count("device_ip='".$s['ip']."'")<=0){
					...
					$server->set_data('groupid', $_POST['groupid'.$i.'1']);
		...
		for($i=0; $i<count($_POST['enable']); $i++){
			switch($_POST['enable'][$i]){
				...
				case 'superpassword':
					...
					$set .='superpassword=udf_encrypt("'.get_request('superpassword',1,1).'"),';
					break;
				case 'sshport':
