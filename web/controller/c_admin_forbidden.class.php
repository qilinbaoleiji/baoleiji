...
class c_admin_forbidden extends c_base {
	...
	function forbiddengps_save(){
		...
		if($add == 'new'){
			...
			$a = $this->forbiddengps_set->select_all("gname='".$gname."'");
			if(!empty($a[0])){
	...
	function del_forbiddengps() {
		$gid = get_request('gid');
		$this->forbiddengpscommand_set->query("DELETE FROM ".$this->forbiddengpscommand_set->get_table_name()." WHERE gid IN(SELECT gname FROM ".$this->forbiddengps_set->get_table_name()." WHERE gid IN(".(is_array($gid) ? implode(',',$gid) : $gid)."))");
