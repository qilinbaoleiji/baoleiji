...
class c_admin_member extends c_base {
	function index($radiususer=false, $interface=0) {
		...
		if($radiususer){
			...
			$wherelevel = " AND member.level=11";
			$wherelevel_derive = " AND a.level=11";
		}else{
		...
		$allmember = $this->member_set->base_select("SELECT member.* FROM member left join servergroup b on member.groupid=b.id WHERE $where order by $orderby1 $orderby2 LIMIT ".$newpager->intStartPosition.' ,'.$newpager->intItemsPerPage);
	...
	function memberderive($where='1'){
		...
		$result = $this->member_set->base_select("SELECT a.*,b.groupname FROM ".$this->member_set->get_table_name()." a LEFT JOIN ".$this->usergroup_set->get_table_name()." b ON a.groupid=b.id WHERE $where and a.level=0 ORDER BY a.username ASC");
		...
		foreach($result as $info) {
			...
			$sgroup = null;
			$str .= $info['username'].",";
			$str .= stripslashes($info['password']).",";
			if(!$_SESSION['RADIUSUSERLIST']){
				if($_CONFIG['LDAP']){
				...
				$str .= $info['vpnip'].",";
				$str .= "'".$info['usbkey']."',";
				$str .= ($info['localauth'] ? '是' : '否').",";
				$str .= ($info['radiusauth'] ? '是' : '否').",";
				$str .= ($info['ldapauth'] ? '是' : '否').",";
				$str .= ($info['adauth'] ? '是' : '否').",";
				$str .= ($info['auth']==2 ? '是' : '否').",";
				$str .= ($info['authtype'] ? '组合' : '单一').",";
				switch($info['firstauth']){
					case 'localauth':
						$str .= "本地,";
						break;
					case 'radiusauth':
				...
				$str .= ($info['forceeditpassword'] ? '是' : '否').",";
			}
			$str .= "\n";		
	...
	function edit() {
		...
		$allusbkey = $this->usbkey_set->select_all('keyid not in(select usbkey from member where usbkey is not null) or keyid=\''.$member[usbkey].'\'');
		$weektime = $this->weektime_set->select_all();
		$sourceip = $this->sourceip_set->select_all(" sourceip=''");
...
*/		
	...
	function regenerateqrcode(){
		...
		echo '<script>alert("操作成功");window.parent.document.location="admin.php?controller=admin_member&action=edit&uid='.$uid.'"</script>';
	}
	...
	function save() {
		...
		$_SESSION['MEMBERADDTMP']['groupid']=$groupid = intval($_POST['groupid1']);
		...
		for($i=1; true; $i++){
			if($_POST['groupid'.$i]){
				$_SESSION['MEMBERADDTMP']['groupid']=$groupid = $_POST['groupid'.$i];
				continue;
			}			
			break;
		}
		for($i=1; true; $i++){
			if($_POST['mgroupid'.$i]){
				$_SESSION['MEMBERADDTMP']['mgroupid']=$mgroupid = $_POST['mgroupid'.$i];
				continue;
			}			
			break;
		}
		...
		if($rdplocalcheck!=$oldmember['rdplocalcheck']){
			echo '<script>alert("由于更改了默认，需要重启IE")</script>';
		}
			...
			if($uid == 0) {
				...
				if($this->member_set->select_all("username = '" . $newmember->get_data('username') . "'") == NULL ) {
					...
					if(!$_SESSION['RADIUSUSERLIST']){
						...
						if(empty($loguser)){
							if($log_priority>=0){
								$sql = "INSERT INTO ".LOG_DBNAME.".log_user set username='".$newmember->get_data('username')."',password=md5('".$this->member_set->udf_decrypt($newmember->get_data('password'))."'),email='".$newmember->get_data('email')."',level='".$log_priority."'";
								$this->member_set->query($sql);
							}
						}else{
							$sql = "UPDATE ".LOG_DBNAME.".log_user set username='".$newmember->get_data('username')."',password=md5('".$this->member_set->udf_decrypt($newmember->get_data('password'))."'),email='".$newmember->get_data('email')."',level='".$log_priority."' WHERE  userid=$uid";
							$this->member_set->query($sql);
						}
					}
					...
					if($autosetpwd){
						...
						if(!$smtp->conn) echo '<script>密码发送失败，请联系管理员查看邮箱设置</script>';
					...
					if($_SESSION['RADIUSUSERLIST']){
						...
						echo '<script>alert(\'成功添加用户'.($asyncaccount_errorip ? ',同步账号到应用发布服务器:'.implode(' ', $asyncaccount_errorip)." 失败 " : '').'\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=radiusmember&gid='.$gid.'";</script>';
						return $uid;
						//alert_and_back('成功添加用户', 'admin.php?controller=admin_member&action=radiususer');
					}else{
						echo '<script>alert(\'成功添加用户'.($asyncaccount_errorip ? ',同步账号到应用发布服务器:'.implode(' ', $asyncaccount_errorip)." 失败 " : '').'\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=member&gid='.$gid.'";</script>';
				...
				if(1){
					...
					if(empty($loguser)){
						...
						$sql = "UPDATE ".LOG_DBNAME.".log_user set password=md5('".$this->member_set->udf_decrypt($newmember->get_data('password'))."'),level='".$log_priority."',email='".$newmember->get_data('email')."' WHERE  username='".$newmember->get_data('username')."'";
						$this->member_set->query($sql);
					}
					if($log_priority==-1){
						$sql = "DELETE FROM `log_user`  WHERE  username='".$newmember->get_data('username')."'";
						$this->member_set->query($sql);
					}
					if($db_priority==-1){
				...
				if($oldmember['level']==11){
					...
					echo '<script>alert(\'成功修改用户'.($asyncaccount_errorip ? ',同步账号到应用发布服务器:'.implode(' ', $asyncaccount_errorip)." 失败 " : '').'\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=radiusmember&gid='.$gid.'";</script>';
					//exit;
						
					//alert_and_back('成功编辑用户', 'admin.php?controller=admin_member&action=radiususer');
				}else{
					echo '<script>alert(\'成功修改用户'.($asyncaccount_errorip ? ',同步账号到应用发布服务器:'.implode(' ', $asyncaccount_errorip)." 失败 " : '').'\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=member&gid='.$gid.'";</script>';
					//exit;
	...
	function batchadd_save($encrypt=0){
		...
		if($added || $modified){
			...
			if($_SESSION['RADIUSUSERLIST']){
				echo '<script>alert(\''.$msg.'\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=radiusmember&gid='.$gid.'";</script>';
				//alert_and_back($msg,'admin.php?controller=admin_member&action=radiususer');
			}else{
				echo '<script>alert(\''.$msg.'\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=member&gid='.$gid.'";</script>';
	...
	function usergroup_save() {
		...
		if(0 == $this->usergroup_set->select_count("groupname = '$new_name' and id!='$id'")) {
			...
			if($id){
				...
				$adminlog->set_data('resource', '用户'.($levex ? '目录' : '组').':'.$new_name);
				$this->admin_log_set->add($adminlog);
				echo '<script>alert(\'成功添加\');window.parent.menu.location="admin.php?controller=admin_index&action=menu&actions=usergroup&ldapid='.$ldapid.'";</script>';
	...
	function delete() {
		...
		if($_CONFIG['CREATE_LOG_USER'])
		$this->member_set->query("DELETE FROM ".LOG_DBNAME.".log_user WHERE username='".$user['username']."'");
	...
	function save_self() {
		...
		if($newmember->get_errnum() == 0) {
			$loguser = $this->member_set->base_select("SELECT * FROM ".LOG_DBNAME.".log_user where username='".$user['username']."'");
			if(!empty($loguser)){
				$sql = "UPDATE ".LOG_DBNAME.".log_user set password=md5('".$this->member_set->udf_decrypt($newmember->get_data('password'))."'),email='".$newmember->get_data('email')."' WHERE  username='".$user['username']."'";
				$this->member_set->query($sql);
			}
	...
	function delete_all() {
		...
		if($usernames_u){
			...
			$this->member_set->query("DELETE FROM ".LOG_DBNAME.".log_user WHERE username IN ('".implode("','",$usernames_u)."')");
	...
	function keybinduser_save(){
		...
		if($keyid&&empty($member)){
			$newmember = new member();
			$newmember->set_data('uid', $oldusbkeybind[0]['uid']);
			$newmember->set_data('usbkeystatus', 0);
			$this->member_set->edit($newmember);
			if($fromajax){
				echo '解绑成功';
			...
			alert_and_back('解绑成功', 'admin.php?controller=admin_member&action=keybinduser&keyid='.$keyid);
			exit;
		}
		...
		if($this->usbkey_set->query('UPDATE '.$this->member_set->get_table_name().' SET usbkey="'.$key['keyid'].'",usbkeystatus=1 WHERE uid ='.$member)){
			...
			$newmember = new member();
			$newmember->set_data('uid', $member);
			if($usbkeybind[0]['type']&&$user['usbkey']!=$key[keyid]){	
				$newmember->set_data('usbkeystatus', 11);
			}else{
				$newmember->set_data('usbkeystatus', 0);
			}
	...
	function domemberimport(){
		...
		for($i=1; $i<count($lines); $i++){
			...
			if(!$_SESSION['RADIUSUSERLIST']){
				...
				$localauth=$linearr[$index_i++];
				$localauth = ($localauth=='是' ? 1 : 0);
				$radiusauth=$linearr[$index_i++];
				$radiusauth = ($radiusauth=='是' ? 1 : 0);
				$ldapauth=$linearr[$index_i++];
				$ldapauth = ($ldapauth=='是' ? 1 : 0);
				$adauth=$linearr[$index_i++];
				$adauth = ($adauth=='是' ? 1 : 0);
				$auth=$linearr[$index_i++];
				$auth = ($auth=='是' ? 2 : 0);
				$authtype=$linearr[$index_i++];
				$authtype = ($authtype=='组合' ? 1 : 0);
				$firstauth=$linearr[$index_i++];
				switch($firstauth){
					case '本地':
						$firstauth='localauth';
						break;
					case 'Radius':
						$firstauth='radiusauth';
						break;
					case 'radius':
						$firstauth='radiusauth';
						break;
				}
				$forceeditpassword=$linearr[$index_i++];
				$forceeditpassword = ($forceeditpassword=='是' ? 1 : 0);
			}else{
				$username=$linearr[$index_i++];
				$password=$linearr[$index_i++];
				$level = 11;
