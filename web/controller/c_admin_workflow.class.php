...
class c_admin_workflow extends c_base {
	...
	function workflow_edit(){
		...
		$sql  = "select distinct devices.id,devices.device_ip,devices.username,login_template.login_method,sg1.id groupid,sg1.groupname,sg1.level grouplevel,sg1.ldapid groupldapid,sg2.id pgroupid,sg2.groupname pgroupname,sg2.level pgrouplevel,sg2.ldapid pgroupldapid,sg3.id ppgroupid,sg3.groupname ppgroupname,sg3.level ppgrouplevel,sg3.ldapid ppgroupldapid from luser left join member on luser.memberid=member.uid left join devices on luser.devicesid=devices.id left join login_template on devices.login_method=login_template.id LEFT JOIN servers ON devices.device_ip=servers.device_ip LEFT JOIN servergroup sg1 ON servers.groupid=sg1.id LEFT JOIN servergroup sg2 ON sg1.ldapid=sg2.id LEFT JOIN servergroup sg3 ON sg2.ldapid=sg3.id where member.uid and luser.devicesid AND $where";		
		$sql .= " union select distinct devices.id,devices.device_ip,devices.username,login_template.login_method,sg1.id groupid,sg1.groupname,sg1.level grouplevel,sg1.ldapid groupldapid,sg2.id pgroupid,sg2.groupname pgroupname,sg2.level pgrouplevel,sg2.ldapid pgroupldapid,sg3.id ppgroupid,sg3.groupname ppgroupname,sg3.level ppgrouplevel,sg3.ldapid ppgroupldapid from luser_resourcegrp a left join (select a.id,b.devicesid from resourcegroup a left join resourcegroup b on a.groupname=b.groupname where a.devicesid=0 ) t on a.resourceid=t.id left join member on a.memberid=member.uid left join devices on t.devicesid=devices.id left join login_template on devices.login_method=login_template.id LEFT JOIN servers ON devices.device_ip=servers.device_ip LEFT JOIN servergroup sg1 ON servers.groupid=sg1.id LEFT JOIN servergroup sg2 ON sg1.ldapid=sg2.id LEFT JOIN servergroup sg3 ON sg2.ldapid=sg3.id where t.id and member.uid and t.devicesid AND $where";
		$sql .= " union select distinct devices.id,devices.device_ip,devices.username,login_template.login_method,sg1.id groupid,sg1.groupname,sg1.level grouplevel,sg1.ldapid groupldapid,sg2.id pgroupid,sg2.groupname pgroupname,sg2.level pgrouplevel,sg2.ldapid pgroupldapid,sg3.id ppgroupid,sg3.groupname ppgroupname,sg3.level ppgrouplevel,sg3.ldapid ppgroupldapid from lgroup left join member on lgroup.groupid=member.groupid left join devices on lgroup.devicesid=devices.id left join login_template on devices.login_method=login_template.id LEFT JOIN servers ON devices.device_ip=servers.device_ip LEFT JOIN servergroup sg1 ON servers.groupid=sg1.id LEFT JOIN servergroup sg2 ON sg1.ldapid=sg2.id LEFT JOIN servergroup sg3 ON sg2.ldapid=sg3.id where member.uid and lgroup.devicesid AND $where";
		$sql .= " union select distinct devices.id,devices.device_ip,devices.username,login_template.login_method,sg1.id groupid,sg1.groupname,sg1.level grouplevel,sg1.ldapid groupldapid,sg2.id pgroupid,sg2.groupname pgroupname,sg2.level pgrouplevel,sg2.ldapid pgroupldapid,sg3.id ppgroupid,sg3.groupname ppgroupname,sg3.level ppgrouplevel,sg3.ldapid ppgroupldapid from lgroup_resourcegrp a left join (select a.id,b.devicesid from resourcegroup a left join resourcegroup b on a.groupname=b.groupname where a.devicesid=0 ) t on a.resourceid=t.id left join member on a.groupid=member.groupid left join devices on t.devicesid=devices.id left join login_template on devices.login_method=login_template.id LEFT JOIN servers ON devices.device_ip=servers.device_ip LEFT JOIN servergroup sg1 ON servers.groupid=sg1.id LEFT JOIN servergroup sg2 ON sg1.ldapid=sg2.id LEFT JOIN servergroup sg3 ON sg2.ldapid=sg3.id where t.id and member.uid and t.devicesid AND $where ORDER BY device_ip ASC,login_method ASC ";
		...
		for($i=0; $i<count($userpriority); $i++){
			...
			if(!$found1&&$userpriority[$i]['groupid']){
				$allsgroup[] = array('id'=>$userpriority[$i]['groupid'],'groupname'=>$userpriority[$i]['groupname'],'ldapid'=>$userpriority[$i]['groupldapid'],'level'=>$userpriority[$i]['grouplevel']);
			}
			if(!$found2&&$userpriority[$i]['pgroupid']){
				$allsgroup[] = array('id'=>$userpriority[$i]['pgroupid'],'groupname'=>$userpriority[$i]['pgroupname'],'ldapid'=>$userpriority[$i]['pgroupldapid'],'level'=>$userpriority[$i]['pgrouplevel']);
			}
			if(!$found3&&$userpriority[$i]['ppgroupid']){
				$allsgroup[] = array('id'=>$userpriority[$i]['ppgroupid'],'groupname'=>$userpriority[$i]['ppgroupname'],'ldapid'=>$userpriority[$i]['ppgroupldapid'],'level'=>$userpriority[$i]['ppgrouplevel']);
			}
	...
	function approvedesc(){
		...
		$logs = $this->workflow_log_set->base_select("SELECT member.username,wf.apply_status,wf.apply_date,wf.desc FROM workflow_log wf LEFT JOIN member ON wf.member=member.uid WHERE wid=$wid ORDER BY wf.sid ASC");
		if($logs[count($logs)-1]['username']==$_SESSION['ADMIN_USERNAME'] && $wf['contant']!=2){
			$this->assign("last", 1);
		}
		$this->assign("wid", $wid);
	...
	function workflow_delete(){
		...
		if($error){
			alert_and_back('删除失败:\n'.implode('\n', $error),'admin.php?controller=admin_workflow&action=workflow');
			exit;
		}
		alert_and_back('删除成功','admin.php?controller=admin_workflow&action=workflow');
	}
	...
	function do_workflow_approve(){
		...
		if($status==1){
			if($wf[0]['member']==$_SESSION['ADMIN_UID']&&count($wf)>1)
				$this->workflow_log_set->query("UPDATE ".$this->workflow_set->get_table_name()." SET status=3 WHERE sid=$wid");
			else if($wf[count($wf)-1]['member']==$_SESSION['ADMIN_UID']){
				if($operator){
					$m = $this->member_set->select_by_id($operator);
					$wfdetail = $this->workflow_set->select_by_id($wid);
					$devicesid = $wfdetail['devicesid'];
					...
					if(empty($row)){
						$this->luser_set->query("INSERT INTO luser SET devicesid=".$devicesid.", memberid=".$operator);
					}
				}
