...
class c_admin_status extends c_base {
	...
	function latest(){
 	...
 	//	print_r($latest);
		...
		$this->assign('ha', $ha);
		$querystring = array('controller'=>'admin_status', 'action'=>'latest', 'page'=>$_GET['page']);
		require_once(ROOT ."./controller/c_admin_session.class.php");	
		$session = new c_admin_session;
		$session->init($this->smarty, $this->config);
		$querystring['item']=1;
		arsort($querystring,SORT_STRING);
		$_SERVER['QUERY_STRING'] = 'controller=admin_status&action=latest&item=1&page='.$_GET['page'];//http_build_query($querystring);
		$this->assign('ssh', $session->gateway_running_list(1,'ssh'));
		$querystring['item']=2;
		arsort($querystring,SORT_STRING);
		$_SERVER['QUERY_STRING'] = 'controller=admin_status&action=latest&item=2&page='.$_GET['page'];//http_build_query($querystring);
		$this->assign('telnets', $session->gateway_running_telnet(1,'telnet'));
		require_once(ROOT ."./controller/c_admin_rdprun.class.php");	
		$session = new c_admin_rdprun;
		$session->init($this->smarty, $this->config);
		$querystring['item']=3;
		arsort($querystring,SORT_STRING);
		$_SERVER['QUERY_STRING'] = 'controller=admin_status&action=latest&item=3&page='.$_GET['page'];//http_build_query($querystring);
		$this->assign('rdprun', $session->index(null,1,'rdprun'));
		require_once(ROOT ."./controller/c_admin_vncrun.class.php");	
		$session = new c_admin_vncrun;
		$session->init($this->smarty, $this->config);
		$querystring['item']=4;
		arsort($querystring,SORT_STRING);
		$_SERVER['QUERY_STRING'] = 'controller=admin_status&action=latest&item=4&page='.$_GET['page'];//http_build_query($querystring);
		$this->assign('vncrun', $session->index(null,1,'vncrun'));
		require_once(ROOT ."./controller/c_admin_apppubrun.class.php");	
		$session = new c_admin_apppubrun;
		$session->init($this->smarty, $this->config);
		$querystring['item']=5;
		arsort($querystring,SORT_STRING);
		$_SERVER['QUERY_STRING'] = 'controller=admin_status&action=latest&item=5&page='.$_GET['page'];//http_build_query($querystring);
		$this->assign('apppubrun', $session->index(null,1,'apppubrun'));
