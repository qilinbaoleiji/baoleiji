...
class c_admin_config extends c_base {
	...
	function apppubexport(){
		...
		$result = $this->apppub_set->base_select("SELECT a.username,a.device_ip,c.groupid,udf_decrypt(a.cur_password) password,b.name,b.path,b.appprogramname,b.url,b.description FROM ".$this->appdevice_set->get_table_name()." a LEFT JOIN ".$this->apppub_set->get_table_name()." b ON a.apppubid=b.id LEFT JOIN ".$this->server_set->get_table_name()." c ON a.device_ip=c.device_ip WHERE $where");
			...
			if($newappdeviceid->get_errnum() == 0) {
				if(empty($id)){
					...
					for($i=0; $i<count($_POST['group']); $i++){
						...
						$adminlog = new admin_log();
						$adminlog->set_data('administrator', $_SESSION['ADMIN_USERNAME']);
						$adminlog->set_data('luser', $ugroup['GroupName'].'(运维组)');
					...
					for($i=0; $i<count($added); $i++){
						...
						$adminlog = new admin_log();
						$adminlog->set_data('administrator', $_SESSION['ADMIN_USERNAME']);
						$adminlog->set_data('luser', $ugroup['GroupName'].'(资源组)');
						$adminlog->set_data('action', language('应用发布绑定资源组'));
						$adminlog->set_data('resource', $apppub['name']);
						$this->admin_log_set->add($adminlog);
						unset($adminlog);
	...
	function config_ftp()
	{
		...
		$DiskFull = $this->setting_set->select_all("sname='DiskFull'");
		$DiskFull = $DiskFull[0]['svalue'];
		...
		$this->assign("asyncoutpass", $AsyncoutPass);
	...
	function dothreepripwd_twopwd(){
		...
		if($this->member_set->select_count("username='audit' and password=udf_encrypt('".$auditpassword."')")<=0){
			echo '<script>alert("audit用户密码不正确");</script>';
			exit;
		}
		if($this->member_set->select_count("username='password' and password=udf_encrypt('".$passwordpassword."')")<=0){
	...
	function certificate_save(){
		...
		system('sudo /usr/bin/killall -1 httpd');
	...
	function ldapusers(){
		...
		if($_POST['address']){
			...
			if ($ldapconn) {
				...
				$_ou = explode(',', $_POST['ou']);
				$sou = '';
				for($j=0; $j<count($_ou); $j++){
					$sou .= "ou=".$_ou[$j].',';
				}
				...
				if ($ldapbind) {
					$basedn = $sou.$sdomain;
					$justthese = array("uid","cn","sn");
					$sr=ldap_search($ldapconn, $basedn, "(&(objectClass=person))",$justthese , 0, 0, 0);
					$info = ldap_get_entries($ldapconn, $sr);
					$nogroupusers = array();
					for($i=0; $i<$info['count']; $i++){
						$info[$i]['uid'][0]=trim($info[$i]['uid'][0]);
						if(!empty($info[$i]['uid'][0])&&$this->member_set->select_count("username='".trim($info[$i]['uid'][0])."'")==0)
						$nogroupusers[]=array('name'=>trim($info[$i]['cn'][0]), 'username'=>trim($info[$i]['uid'][0]));
					}
	...
	function adusers(){
		...
		$pwdconfig = unserialize($setting[0]['svalue']);
		//$this->assign("ldaps", $pwdconfig['ldapconfig']);
		$this->assign("adconfig", $pwdconfig['adconfig'][$id]);
		...
		if($_POST['address']||($_SESSION['AD_address']&&$_SESSION['AD_domain']&&$_SESSION['AD_adusername']&&$_SESSION['AD_adpassword'])){
			...
			try {
				...
				$sdomain = substr($sdomain, 0, strlen($sdomain)-1);
				$options['base_dn'] = (empty($_POST['ou']) ? '' : $_POST['ou'].',').$sdomain;
			...
			$_SESSION['AD_dn']=$options['base_dn'];
			...
			for($i=0; $i<$r['count']; $i++){
				...
				if(strtolower($distinguishedname)==strtolower($options['base_dn'])){//echo '<pre>';var_dump($r[$i]);echo '</pre>';echo '------------------------------------------------';
					if($r[$i]['ou'][0]) $ous[] = $r[$i]['ou'][0];
					else $cns[] = $r[$i]['cn'][0];
				}
			}
			//echo '<pre>';var_dump($r);echo '</pre>';echo 'netmwd';


			$allusers = array(-1);
			foreach($adldap->user()->all() as $val)
			{
				$distinguishedname=$adldap->user()->dn($val);
			...
			//echo '<pre>';var_dump($allusers);echo '</pre>';echo 'netmwd';
...
//	echo '<pre>';var_dump($adldap->group()->members('ceshi3g'));echo '</pre>';
			...
			{	
				$distinguishedname=$adldap->group()->info($val);
				$distinguishedname = substr($distinguishedname[0]['distinguishedname'][0],strpos($distinguishedname[0]['distinguishedname'][0],',')+1);
				if(strtolower($distinguishedname)==strtolower($options['base_dn'])){				
					$users = $adldap->group()->members($val);//var_dump($users);
					$_users = array();
					for($i=0; $i<count($users); $i++){
						if(in_array($users[$i], $allusers)){							
							$_users[]=array('username'=>$users[$i], 'exists'=>(in_array($users[$i], $_users) ? 1 : 0));
						}
					}//var_dump($_users);
					if(count($_users)>0){
						sort($_users);
...
*/
			...
			$this->assign("step", 3);
			$this->adusers_check();
			$this->assign("ous", $_SESSION['ADUSERS_OU']);
			$this->assign("groups", $this->sgroup_set->select_all("level=0", 'groupname', 'ASC'));
		}
	...
	function adusersbygroup(){
		...
		if($groupname){
			...
			$_allusers = null;
			sort($allusers);
			echo json_encode(array('dn'=>$options['base_dn'],'pid'=>$pid,'ous'=>array(),'groups'=>array(),'users'=>$allusers));
			/*
			for($i=0; $i<count($allusers); $i++){
				echo $allusers[$i].","."0\r\n";
			}*/
	...
	function adusers_save(){
		...
		for($i=0; $i<count($_POST['username']); $i++){			
			...
			$_POST['groupid'][$i]=intval($_POST['g_id']);
	...
	function adimportconfig(){
		...
		$configs = $this->adimportconfig_set->select_all("1", "title", "asc");
		$config = $configs[0];
		if($id){
	...
	function synchronization_ad_users($id=null){
		...
		if($allusers){
			...
			$disabledusers = $this->member_set->select_all("(adou!='' or adou is not null) and level=0 and enable=1 and username NOT IN('".implode("','", $allusers)."')");
