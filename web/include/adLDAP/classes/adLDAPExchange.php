...
class adLDAPExchange {
    ...
    */
    public function servers($attributes = array('cn','distinguishedname','serialnumber')) 
    {
        ...
        $sr = @ldap_search($this->adldap->getLdapConnection(), $configurationNamingContext[0]['configurationnamingcontext'][0],'(&(objectCategory=msExchExchangeServer))', $attributes,0,0);
    ...
    */
    public function storageGroups($exchangeServer, $attributes = array('cn','distinguishedname'), $recursive = NULL) 
    {
        ...
        $sr = @ldap_search($this->adldap->getLdapConnection(), $exchangeServer, $filter, $attributes,0,0);
        $entries = @ldap_get_entries($this->adldap->getLdapConnection(), $sr);
    ...
    */
    public function storageDatabases($storageGroup, $attributes = array('cn','distinguishedname','displayname')) {
        ...
        $sr = @ldap_search($this->adldap->getLdapConnection(), $storageGroup, $filter, $attributes,0,0);
        $entries = @ldap_get_entries($this->adldap->getLdapConnection(), $sr);
        return $entries;
    }
}
