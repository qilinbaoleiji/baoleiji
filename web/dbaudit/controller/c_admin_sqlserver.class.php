...
class c_admin_sqlserver extends c_base {
	function index($where = NULL) {
		...
		if($delete) {
			...
			$sql = "show tables like '".$this->sqlserver_set->get_table_name().'20%\'';
			 $tnames = $this->sqlserver_set->base_select($sql);
			 $ttnames = null;
			...
			if($tablename){
				$this->sqlserver_set->set_table_name($tablename);
	...
	function detail(){
		...
		if($tablename)
		$this->sqlserver_set->set_table_name($tablename);
		$detail = $this->sqlserver_set->select_by_id($sid);
		$this->assign("detail", $detail);
		$this->display("sqlserversession_detail.tpl");
	}
	...
	function cmddetail(){
		...
		$tablename = get_request("tablename", 0, 1);
		if($tablename)
		$this->sqlservercommands_set->set_table_name($tablename);
		$detail = $this->sqlservercommands_set->select_by_id($cid);
		$this->assign("detail", $detail);
		$this->display("sqlserverview_detail.tpl");
	}
	...
	function view() {
		...
		$this->display('sqlserversession_view.tpl');
	}
