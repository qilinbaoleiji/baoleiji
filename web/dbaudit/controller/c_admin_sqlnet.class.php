...
class c_admin_sqlnet extends c_base {
	...
	function index($where = NULL) {
		...
		$tablename = get_request('tablename', 0, 1);
		...
		$login_success = get_request('login_success', 0, 1);
		$login_success_set = get_request('login_success_set', 0, 1);
		...
		if($login_success_set){
			if($login_success) {
				$where .= " AND login_success = 1";
			}
			else {
				$where .= " AND login_success = 0";
			}
		}
		...
		if($delete) {
			...
			//echo $curr_url;
			$sql = "show tables like '".$this->sqlnet_set->get_table_name().'20%\'';
			 $tnames = $this->sqlnet_set->base_select($sql);
			 $ttnames = null;
			 for($j=0; $j<count($tnames); $j++){
				 $keys =  key($tnames[$j]);//var_dump($keys);
				 $datatables[$j]['n']=$tnames[$j][$keys];
			 }
			 $this->assign('datatables', $datatables);
			$ymd = substr($tablename, strlen($tablename)-8);
			$ctable = substr($tablename,0, strpos($tablename, "_")+1).'commands'.$ymd;
			if($tablename){
				$this->sqlnet_set->set_table_name($tablename);
	...
	function detail(){
		...
		if($tablename)
		$this->sqlnet_set->set_table_name($tablename);
		$detail = $this->sqlnet_set->select_by_id($sid);
		$this->assign("detail", $detail);
		$this->display("sqlnetsession_detail.tpl");
	}
	...
	function cmddetail(){
		...
		$tablename = get_request("tablename", 0, 1);
		if($tablename)
		$this->sqlcommands_set->set_table_name($tablename);
		$detail = $this->sqlcommands_set->select_by_id($cid);
		$this->assign("detail", $detail);
		$this->display("sqlnetview_detail.tpl");
	}
