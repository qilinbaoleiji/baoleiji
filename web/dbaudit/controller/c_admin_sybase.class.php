...
class c_admin_sybase extends c_base {
	function index($where = NULL) {
		...
		if($delete) {
			...
			//echo $curr_url;
			$sql = "show tables like '".$this->sybase_set->get_table_name().'20%\'';
			$tnames = $this->sybase_set->base_select($sql);
			$ttnames = null;
			...
			if($tablename){
				$this->sybase_set->set_table_name($tablename);
	...
	function detail(){
		...
		if($tablename)
		$this->sybase_set->set_table_name($tablename);
		$detail = $this->sybase_set->select_by_id($sid);
		$this->assign("detail", $detail);
		$this->display("sybasesession_detail.tpl");
	}
	...
	function view() {
		...
		if($tablename){
			$this->sybasecommands_set->set_table_name($tablename);
		}
