...
class c_admin_mysql extends c_base {
	function index($where = NULL) {
		...
		if($delete) {
			...
			//echo $curr_url;
			 ...
			 $sql = "show tables like '".$this->mysql_set->get_table_name().'20%\'';
			 $tnames = $this->mysql_set->base_select($sql);
			 $ttnames = null;
			...
			if($tablename){
				$this->mysql_set->set_table_name($tablename);
	...
	function detail(){
		...
		if($tablename)
		$this->mysql_set->set_table_name($tablename);
		$detail = $this->mysql_set->select_by_id($sid);
		$this->assign("detail", $detail);
		$this->display("mysqlsession_detail.tpl");
	}
	...
	function view() {
		...
		if($tablename){
			$this->mysqlcommands_set->set_table_name($tablename);
		}
		...
		//echo '<pre>';var_dump($allcommand);echo '</pre>';
		for($j = 0; $j < count($allcommand); $j++) {
			if(empty($allcommand[$j]['return_code'])){
				$allcommand[$j]['return_code']='NULL';
			}
			if($command){
