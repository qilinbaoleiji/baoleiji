...
class c_admin_db2 extends c_base {
	function index($where = NULL) {
		...
		if($delete) {
			...
			//echo $curr_url;
			 ...
			 $sql = "show tables like '".$this->db2_set->get_table_name().'20%\'';
			 $tnames = $this->db2_set->base_select($sql);
			 $ttnames = null;
			...
			if($tablename){
				$this->db2_set->set_table_name($tablename);
	...
	function detail(){
		...
		if($tablename)
		$this->db2_set->set_table_name($tablename);
		$detail = $this->db2_set->select_by_id($sid);
		$this->assign("detail", $detail);
		$this->display("db2session_detail.tpl");
	}
	...
	function view() {
		...
		if($tablename){
			$this->db2cmmands_set->set_table_name($tablename);
		}
