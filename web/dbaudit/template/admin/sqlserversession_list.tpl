...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_sqlserver&orderby1=s_addr&orderby2={{$orderby2}}">来源地址</a></th>
						...
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_sqlserver&orderby1=danger&orderby2={{$orderby2}}">危险级别</a></th>
					...
					<tr {{if $allsession[t].dangerous > 5}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td><a href="admin.php?controller=admin_sqlserver&s_addr={{$allsession[t].s_addr}}">{{$allsession[t].s_addr}}</a></td>
						...
						<td>{{if $allsession[t].danger eq 1}}一般风险{{elseif $allsession[t].danger eq 2}}关注行为{{elseif $allsession[t].danger eq 3}}低风险{{elseif $allsession[t].danger eq 4}}中风险{{elseif $allsession[t].danger eq 5}}高风险{{/if}}</td>	
						<td style="TEXT-ALIGN: left;"><img src="{{$template_root}}/images/ico2.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_sqlserver&action=view&sid={{$allsession[t].sid}}&command={{$command}}&tablename={{$ctable}}">命令</a>({{if $allsession[t].commands}}{{$allsession[t].commands}}{{else}}0{{/if}})&nbsp;<!--|<img src="{{$template_root}}/images/doc_table.gif" width="16" height="16" align="absmiddle">
						<a style="cursor:hand" onclick="javascript:window.open('admin.php?controller=admin_sqlserver&action=detail&sid={{$allsession[t].sid}}&tablename={{$tablename}}','newwin')" >详细</a>-->{{if $admin_level == 2}} &nbsp; | <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_sqlserver&action=del_session&sid={{$allsession[t].sid}}">删除</a>{{/if}}</td>
					</tr>
					{{/section}}
					<tr>
