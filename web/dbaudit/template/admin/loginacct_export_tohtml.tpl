  ...
  <tr>
	<td class="main_content">
		<table bordercolor="white" cellspacing="1" cellpadding="5" border="0" width="100%"  class="BBtable">
					<tr>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">日期</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">服务器</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="6%">数据库类型</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="15%">用户</th>
						<th class="list_bg"  bgcolor="d9ecfa" width="8%">登录次数</th>
					</tr>
					{{section name=t loop=$alllog}}
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						<td><a href="admin.php?controller=admin_reports&action=loginacct&from={{$alllog[t].sourceip}}">{{$alllog[t].day}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginacct&auditip={{$alllog[t].auditip}}">{{$alllog[t].ip}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginacct&serverip={{$alllog[t].serverip}}">{{$alllog[t].db_type}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginacct&portocol={{$alllog[t].portocol}}">{{$alllog[t].user}}</a></td>
						<td><a href="admin.php?controller=admin_reports&action=loginacct&audituser={{$alllog[t].audituser}}">{{$alllog[t].total}}</a></td>
					</tr>
					{{/section}}
