...
cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						...
						<th class="list_bg"   width="10%"><a href="admin.php?controller=admin_sybase&orderby1=commands&orderby2={{$orderby2}}">命令条数</a></th>
					...
					<tr {{if $allsession[t].dangerous > 5}}bgcolor="red"{{elseif $allsession[t].dangerous > 0}}bgcolor="yellow" {{elseif $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						...
						<td>{{$allsession[t].commands}}</td>
						<td style="TEXT-ALIGN: left;"><img src="{{$template_root}}/images/ico2.gif" width="16" height="16" align="absmiddle">
						<a href="admin.php?controller=admin_sybase&action=view&sid={{$allsession[t].sid}}&command={{$command}}&tablename={{$ctable}}">命令</a>({{if $allsession[t].commands}}{{$allsession[t].commands}}{{else}}0{{/if}})&nbsp;<!--|<img src="{{$template_root}}/images/doc_table.gif" width="16" height="16" align="absmiddle">
						<a style="cursor:hand" onclick="javascript:window.open('admin.php?controller=admin_sybase&action=detail&sid={{$allsession[t].sid}}&tablename={{$tablename}}','newwin')" >详细</a>-->{{if $admin_level == 2}} | <img src="{{$template_root}}/images/scico.gif" width="16" height="16" align="absmiddle"><a href="admin.php?controller=admin_sybase&action=del_session&sid={{$allsession[t].sid}}">删除</a>{{/if}}</td>
					</tr>
					{{/section}}
					<tr>
