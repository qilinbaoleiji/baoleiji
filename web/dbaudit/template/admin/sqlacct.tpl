...
function searchit(){
	document.search.action = "admin.php?controller=admin_sqlaccount&action={{$action}}";
...
<form action="admin.php?controller=admin_sqlaccount&action={{$action}}" method="post" name="search" >
...
//cal.manageFields("f_rangeEnd_trigger", "f_rangeEnd", "%Y-%m-%d %H:%M:%S");
  ...
  <tr><td><table bordercolor="white" cellspacing="0" cellpadding="5" border="0" width="100%" class="BBtable">
					<tr>
						...
						<th class="list_bg"   width="15%"><a href="admin.php?controller=admin_loginaccount&action={{$action}}&orderby1=sqlcmd&orderby2={{$orderby2}}" >sql指令</a></th>
					</tr>
					...
					<tr {{if $smarty.section.t.index % 2 == 0}}bgcolor="f7f7f7"{{/if}}>
						...
						<td><a href="admin.php?controller=admin_loginaccount&action={{$action}}&sqlcmd={{$alllog[t].sqlcmd}}">{{$alllog[t].sqlcmd}}</a></td>
					</tr>
					{{/section}}
					<tr>
