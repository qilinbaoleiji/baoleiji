...
//var_dump(empty($minfo['webportal']) && (mktime()-$_SESSION['startonlinetime'] > $pwdconfig['logintimeout']*60));
//var_dump((!empty($minfo['webportal'])&&$minfo['webportallogin']+(empty($minfo['webportaltime']) ? 24*60 : $minfo['webportaltime'])*60<mktime()));
$sessionexpired = (empty($minfo['webportal']) && (mktime()-$_SESSION['startonlinetime'] > $pwdconfig['logintimeout']*60) || (!empty($minfo['webportal'])&&$minfo['webportallogin']<mktime()));
...
if($_GET['controller']=='admin_interface'&&$_POST['sn']==$_CONFIG['interfacesn']){
...
if($_SESSION['ADMIN_UID']){
	...
	if(!isset($_SESSION['JUSTLOGINSHOWPWDEDIT'])&&((mktime()-$minfo['lastdateChpwd'])/(24*3600) > ($pwdconfig['pwdexpired']-$pwdconfig['pwdahead'])||$minfo['forceeditpassword'])&&$action!='menu'&&!($controller=='c_admin_index'&&($action=='index' || $action=='changerole'))&&$action!='save_self'&&$action!='logout'&&$action!='doqrcode'&&$action!='get_sms'&&$action!='login_tip'&&$action!='get_user_login_fristauth'&&$_SESSION['authtype']=='localauth'){
	...
	if(((mktime()-$minfo['lastdateChpwd'])/(24*3600) > ($pwdconfig['pwdexpired']/*-$pwdconfig['pwdahead']*/)||$minfo['forceeditpassword'])&&$action!='menu'&&!($controller=='c_admin_index'&&($action=='index' || $action=='changerole'))&&$action!='save_self'&&$action!='logout'&&$action!='doqrcode'&&$action!='get_sms'&&$action!='login_tip'&&$action!='get_user_login_fristauth'&&$_SESSION['authtype']=='localauth'){
			$controller = 'c_admin_member';
			$action = 'edit_self';
			$_GET['msg']='changepwd';
	}
	$pwdremain = round($pwdconfig['pwdexpired'] - (mktime()-$minfo['lastdateChpwd'])/(24*3600),0);
	$_CONFIG['PWD_REMAIN_DAYS'] = $pwdremain >=0 ? $pwdremain.'天' : '<font color="red">已过期</font>';

	$cologinauth = (($minfo['usbkey']&&$minfo['usbkeystatus']==11) || $minfo['authtype']&&($minfo['radiusauth']||$minfo['ldapauth']||$minfo['adauth']||intval($minfo['auth'])==2));
	if($cologinauth&&empty($_SESSION['USER_MULTI_LOGIN_VALIDATED'])&&$action!='menu'&&!($controller=='c_admin_index'&&($action=='index'))&&$action!='logout'&&$action!='doqrcode'&&$action!='get_sms'&&$action!='login_tip'&&$action!='get_user_login_fristauth'&&$action!='save_self'){
			$controller = 'c_admin_index';
			$action = 'qrcode';
	}
...
if(file_exists(ROOT ."./controller/$controller.class.php")) {
	...
	if((!isset($_SESSION["ADMIN_LOGINED"]) || $_SESSION["ADMIN_LOGINED"] == false) && ($action != 'login_user_field' && $action != 'login' && $action !='chklogin' && $action != 'getpwd'&& $action != 'docronreports'&& $action != 'synchronization_ad_users'&&$action!='get_user_login_fristauth'&& $action != 'get_sms')) {
		...
		$member   = new member_set();
		if($_SESSION['LOG_CONFIG_ON']){
			$loguser = $member->base_select("SELECT * FROM ".LOG_DBNAME.".log_user where username='".$_SESSION["ADMIN_USERNAME"]."'");
			if($loguser){
			//$_SESSION['LOG_ON']=$_CONFIG['LOG_ON']&&($_SESSION['ADMIN_LEVEL'] != 10 ? 1 : 0);
			$_SESSION['LOG_ON']=1;
			}
		}
		if($_SESSION['DBAUDIT_CONFIG_ON']){
			$_dbuser = $member->base_select("SELECT * FROM ".DBAUDIT_DBNAME.".member where username='".$_SESSION["ADMIN_USERNAME"]."'");
			if($_dbuser){
			$_SESSION['DBAUDIT_ON']=1;
			}
		}
		...
		if(method_exists($newcontroller, $action)) {
			...
			$update_group_cache = ($controller='c_admin_member'&&(in_array($action, array('save','delete','batchadd_save','domemberimport')))||$controller='c_admin_pro'&&(in_array($action, array('dev_save','dev_del','batchdevsave','dodevimport','groupadddev_save','dev_group_save','dev_group'))));
